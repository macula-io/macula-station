...
- [`docs/DHT_FIND_FLAKE_ATTEMPT.md`](docs/DHT_FIND_FLAKE_ATTEMPT.md) — investigation of the cross-station DHT find ~60% flake. Attempt 1 (commit `c08ef8d`, reverted in `4bccf1c`) tried to filter daemon entries out of the routing table by observing only outbound peers; collapsed routing tables to ~3 entries each, regressed hit rate from 2/5 to 0/5. Real fix needs multi-round Kademlia iterative or wider eager replication; documented for next attempt.
