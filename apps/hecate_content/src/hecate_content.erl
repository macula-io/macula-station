...
-type mcid()       :: hecate_content_manifest:mcid().
-type manifest()   :: hecate_content_manifest:manifest().
-type store_opts() :: #{
    name           => binary(),
    chunk_size     => pos_integer(),
    hash_algorithm => hecate_content_hasher:algorithm()
}.
...
mcid(Data) ->
    mcid(Data, hecate_content_hasher:default_algorithm()).

-spec mcid(binary(), hecate_content_hasher:algorithm()) -> mcid().
mcid(Data, Algorithm) ->
    Hash = hecate_content_hasher:hash(Algorithm, Data),
    <<1, 16#55, Hash/binary>>.

-spec mcid_to_string(mcid()) -> binary().
mcid_to_string(MCID) ->
    hecate_content_manifest:mcid_to_string(MCID).

-spec mcid_from_string(binary()) -> {ok, mcid()} | {error, invalid_mcid}.
mcid_from_string(String) ->
    hecate_content_manifest:mcid_from_string(String).
...
create_manifest(Data) ->
    hecate_content_manifest:create(Data).

-spec create_manifest(binary(), store_opts()) -> {ok, manifest()}.
create_manifest(Data, Opts) ->
    hecate_content_manifest:create(Data, Opts).

-spec verify_manifest(manifest(), binary()) -> boolean().
verify_manifest(Manifest, Data) ->
    case hecate_content_manifest:verify(Manifest, Data) of
        ok            -> true;
        {error, _}    -> false
    end.

-spec get_chunks(manifest()) -> [hecate_content_chunker:chunk_info()].
get_chunks(Manifest) ->
    maps:get(chunks, Manifest).

-spec missing_chunks(manifest()) -> [hecate_content_chunker:chunk_info()].
...
store(Data, Opts) ->
    {ok, Manifest} = hecate_content_manifest:create(Data, Opts),
    ok = put_chunks(Data, Manifest),
    ok = hecate_content_store:put_manifest(Manifest),
...
is_local(MCID) ->
    hecate_content_store:has_block(MCID).

-spec status() -> map().
status() ->
    hecate_content_store:stats().
...
put_chunks(Data, #{chunk_size := CS, chunks := Infos}) ->
    {ok, Chunks} = hecate_content_chunker:chunk(Data, CS),
...
fetch_all_chunks([Info | Rest], Acc) ->
    Hash = maps:get(hash, Info),
    MCID = <<1, 16#55, Hash/binary>>,
    case hecate_content_store:get_block(MCID) of
        {ok, Bin}       -> fetch_all_chunks(Rest, [Bin | Acc]);
        {error, _} = E  -> E
    end.

has_chunk_locally(#{hash := Hash}) ->
    hecate_content_store:has_block(<<1, 16#55, Hash/binary>>).
