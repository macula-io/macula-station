...
%% / canonical data.
-module(hecate_content_manifest).
...
-type manifest() :: #{
    ...
    root_hash      := binary(),
    chunks         := [hecate_content_chunker:chunk_info()]
}.
...
%%       (default `hecate_content_chunker:default_chunk_size()')</li>
...
create(Data, Opts) when is_binary(Data), is_map(Opts) ->
    ...
    ChunkSize = maps:get(chunk_size, Opts,
                         hecate_content_chunker:default_chunk_size()),
    Algorithm = maps:get(hash_algorithm, Opts,
                         hecate_content_hasher:default_algorithm()),

    {ok, Chunks} = hecate_content_chunker:chunk(Data, ChunkSize),
    ChunkInfos   = hecate_content_chunker:chunk_info(Chunks, Algorithm),
...
compute_mcid(Body, Algorithm) ->
    ...
    make_mcid(?VERSION, ?CODEC_MANIFEST,
              hecate_content_hasher:hash(Algorithm, Bytes)).
...
parse_mcid_with_codec({ok, Codec}, Version, Hex) ->
    parse_mcid_with_hash(hecate_content_hasher:hex_decode(Hex), Version, Codec);
