-module(hecate_content_manifest_tests).
-include_lib("eunit/include/eunit.hrl").

create_returns_manifest_with_mcid_test() ->
    {ok, M} = hecate_content_manifest:create(<<"hello">>),
...
mcid_is_deterministic_for_same_data_test() ->
    {ok, A} = hecate_content_manifest:create(
                <<"x">>, #{name => <<"n">>, chunk_size => 1024}),
    {ok, B} = hecate_content_manifest:create(
                <<"x">>, #{name => <<"n">>, chunk_size => 1024}),
    ?assertEqual(maps:get(mcid, A), maps:get(mcid, B)).

mcid_changes_with_data_test() ->
    {ok, A} = hecate_content_manifest:create(<<"a">>),
    {ok, B} = hecate_content_manifest:create(<<"b">>),
    ?assertNotEqual(maps:get(mcid, A), maps:get(mcid, B)).

encode_decode_roundtrip_test() ->
    {ok, M} = hecate_content_manifest:create(<<"hello world">>),
    {ok, Bin} = hecate_content_manifest:encode(M),
    {ok, M2}  = hecate_content_manifest:decode(Bin),
...
decode_garbage_returns_error_test() ->
    ?assertEqual({error, invalid_manifest},
                 hecate_content_manifest:decode(<<"not valid cbor">>)).

verify_returns_ok_for_matching_data_test() ->
    Data = <<"some content">>,
    {ok, M} = hecate_content_manifest:create(Data),
    ?assertEqual(ok, hecate_content_manifest:verify(M, Data)).

verify_returns_size_mismatch_test() ->
    {ok, M} = hecate_content_manifest:create(<<"abc">>),
    ?assertEqual({error, size_mismatch},
                 hecate_content_manifest:verify(M, <<"abcd">>)).

verify_returns_root_hash_mismatch_test() ->
    {ok, M} = hecate_content_manifest:create(<<"abcde">>),
    %% Same size, different content.
    ?assertEqual({error, root_hash_mismatch},
                 hecate_content_manifest:verify(M, <<"xyzab">>)).

mcid_to_string_format_test() ->
    {ok, M} = hecate_content_manifest:create(<<"hi">>),
    Str = hecate_content_manifest:mcid_to_string(maps:get(mcid, M)),
    ?assertMatch(<<"mcid1-manifest-blake3-", _/binary>>, Str).

mcid_string_roundtrip_test() ->
    {ok, M} = hecate_content_manifest:create(<<"r">>),
    MCID = maps:get(mcid, M),
    Str  = hecate_content_manifest:mcid_to_string(MCID),
    ?assertEqual({ok, MCID}, hecate_content_manifest:mcid_from_string(Str)).

mcid_from_string_invalid_test() ->
    ?assertEqual({error, invalid_mcid},
                 hecate_content_manifest:mcid_from_string(<<"not-an-mcid">>)).

get_chunk_mcid_returns_raw_codec_test() ->
    Data = iolist_to_binary([<<C:8>> || C <- lists:seq(1, 50)]),
    {ok, M} = hecate_content_manifest:create(Data, #{chunk_size => 10}),
    {ok, ChunkMCID} = hecate_content_manifest:get_chunk_mcid(M, 0),
...
get_chunk_mcid_invalid_index_test() ->
    ...
    ?assertEqual({error, invalid_index},
                 hecate_content_manifest:get_chunk_mcid(M, 99)).
