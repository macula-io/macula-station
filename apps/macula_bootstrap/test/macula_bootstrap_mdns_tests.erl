-module(macula_bootstrap_mdns_tests).
...
service_name_and_multicast_test() ->
    ?assertEqual("_macula._udp.local",
                 macula_bootstrap_mdns:service_name()),
    ?assertEqual({16#FF02, 0, 0, 0, 0, 0, 0, 16#FB},
                 macula_bootstrap_mdns:multicast_group()),
    ?assertEqual(5353, macula_bootstrap_mdns:multicast_port()).
...
build_query_shape_test() ->
    {Id, Bin} = macula_bootstrap_mdns:build_query(
...
build_query_default_domain_test() ->
    {_Id, Bin} = macula_bootstrap_mdns:build_query(),
...
extract_multiple_txt_records_test() ->
    ...
    Candidates = macula_bootstrap_mdns:extract_candidates(Answers),
...
extract_ignores_non_txt_test() ->
    ...
    ?assertEqual(1, length(macula_bootstrap_mdns:extract_candidates(Answers))).
...
extract_ignores_wrong_name_test() ->
    ...
    ?assertEqual([], macula_bootstrap_mdns:extract_candidates(Answers)).
...
extract_drops_port_out_of_range_test_() ->
    ...
    [?_assertEqual(
        [],
        macula_bootstrap_mdns:extract_candidates(
