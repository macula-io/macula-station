%% EUnit tests for macula_bootstrap_via_operator_paste.
-module(macula_bootstrap_via_operator_paste_tests).

-include_lib("eunit/include/eunit.hrl").

strategy_is_via_operator_paste_test() ->
    ?assertEqual(via_operator_paste, macula_bootstrap_via_operator_paste:strategy()).

stagger_is_zero_test() ->
    ?assertEqual(0, macula_bootstrap_via_operator_paste:stagger_ms()).

discover_without_urls_errors_test() ->
    ?assertEqual({error, no_urls},
                 macula_bootstrap_via_operator_paste:discover(#{})).
...
discover_yields_single_peer_test() ->
    {Url, Kp} = build_url([]),
    {ok, [Peer]} = macula_bootstrap_via_operator_paste:discover(
                     #{peer_urls => [Url]}),
    ?assertEqual(macula_identity:public(Kp), maps:get(node_id, Peer)),
    ?assertEqual(via_operator_paste, maps:get(strategy, Peer)),
    ?assertEqual(macula_bootstrap_via_operator_paste, maps:get(via, Peer)),
...
discover_yields_multiple_peers_test() ->
    ...
    {U2, _} = build_url([]),
    {U3, _} = build_url([]),
    {ok, Peers} = macula_bootstrap_via_operator_paste:discover(
                    #{peer_urls => [U1, U2, U3]}),
...
build_url(Addrs) ->
    ...
    Record = macula_record:sign(
        ...
        Kp),
    {macula_bootstrap_via_operator_paste_peer_url:encode(Record, Addrs), Kp}.
