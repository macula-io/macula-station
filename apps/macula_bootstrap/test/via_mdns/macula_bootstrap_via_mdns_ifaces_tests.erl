-module(macula_bootstrap_via_mdns_ifaces_tests).
...
loopback_is_ineligible_test() ->
    ...
    ?assertNot(macula_bootstrap_via_mdns_ifaces:eligible(Props)).
...
up_multicast_with_ipv6_is_eligible_test() ->
    Props = [{flags, [up, broadcast, running, multicast]},
             {addr,  {192, 168, 1, 1}},
             {addr,  {16#fe80, 0, 0, 0, 1, 2, 3, 4}}],
    ?assert(macula_bootstrap_via_mdns_ifaces:eligible(Props)).
...
describe_picks_first_link_local_test() ->
    ...
    I = macula_bootstrap_via_mdns_ifaces:describe("ifake0", Props),
...
describe_no_link_local_is_undefined_test() ->
    ...
    I = macula_bootstrap_via_mdns_ifaces:describe("ifake1", Props),
...
describe_recognises_all_link_local_prefixes_test() ->
    ...
    [begin
         ...
         I = macula_bootstrap_via_mdns_ifaces:describe("if", Props),
...
list_with_canned_source_filters_and_describes_test() ->
    ...
    Ifaces = macula_bootstrap_via_mdns_ifaces:list(Source),
...
list_with_source_error_returns_empty_test() ->
    ...
    ?assertEqual([], macula_bootstrap_via_mdns_ifaces:list(Source)).
