-module(macula_bootstrap_via_mdns_responder_tests).
...
advertisement_happy_path_test() ->
    ...
    {ok, RespBin} = macula_bootstrap_via_mdns_query:build_advertisement(Query, Info),
...
advertisement_ignores_response_packet_test() ->
    ...
    ?assertEqual(ignore,
                 macula_bootstrap_via_mdns_query:build_advertisement(Response, Info)).

advertisement_ignores_other_service_test() ->
    Info = sample_info(),
    Query = build_query("_other._udp.local", any),
    ?assertEqual(ignore,
                 macula_bootstrap_via_mdns_query:build_advertisement(Query, Info)).
...
advertisement_ignores_garbage_test() ->
    Info = sample_info(),
    ?assertEqual(ignore,
                 macula_bootstrap_via_mdns_query:build_advertisement(
...
responder_loopback_test_() ->
    {timeout, 5,
     fun() ->
        Info = sample_info(),
        {ok, Pid} = macula_bootstrap_via_mdns_responder:start_link(
                      Info#{socket_opener => loopback_opener()}),
        try
            Port = macula_bootstrap_via_mdns_responder:port(Pid),
            ?assert(Port > 0),
            ...
            ?assertMatch({dns_rr, "_macula._udp.local", txt, in, _, _,
                          _, _, _, _}, Answer)
        after
            macula_bootstrap_via_mdns_responder:stop(Pid)
        end
     end}.
    ...
    {timeout, 5,
     fun() ->
        ...
        try
            ...
            ok = macula_bootstrap_via_mdns_responder:set_silent(Pid, true),
            ?assertEqual(timeout,
                         ask_responder_expect_silence(Port, Query, 200)),
            ok = macula_bootstrap_via_mdns_responder:set_silent(Pid, false),
            ?assertMatch(<<_/binary>>, ask_responder(Port, Query))
        after
            macula_bootstrap_via_mdns_responder:stop(Pid)
        end
...
responder_open_failure_stops_init_test() ->
    ...
    try
        ?assertMatch({error, {open_failed, eaddrinuse}},
                     macula_bootstrap_via_mdns_responder:start_link(Opts)),
        drain_exits()
    after
