...
-type siblings() :: #{
    ...
    entries  := [{non_neg_integer(), hecate_dht_entry:entry()}]
}.

-type insert_result() ::
      {admitted, siblings()}
    | {touched,  siblings()}
    | {replaced, Evicted :: hecate_dht_entry:entry(), siblings()}
    | {rejected, siblings()}.
...
-spec new(hecate_dht_xor:id()) -> siblings().
new(Self) ->
    new(Self, ?DEFAULT_S).

-spec new(hecate_dht_xor:id(), pos_integer()) -> siblings().
...
-spec self_id(siblings())  -> hecate_dht_xor:id().
self_id(#{self_id := V})     -> V.
...
-spec members(siblings()) -> [hecate_dht_entry:entry()].
members(#{entries := Es}) -> [E || {_, E} <- Es].

-spec node_ids(siblings()) -> [hecate_dht_xor:id()].
node_ids(S) -> [hecate_dht_entry:node_id(E) || E <- members(S)].

-spec find(hecate_dht_xor:id(), siblings()) ->
        {ok, hecate_dht_entry:entry()} | error.
find(Id, #{entries := Es}) ->
    find_by_id(Id, Es).

-spec contains(hecate_dht_xor:id(), siblings()) -> boolean().
contains(Id, S) ->
    case find(Id, S) of
        {ok, _} -> true;
        error   -> false
...
-spec farthest(siblings()) -> {ok, hecate_dht_entry:entry()} | error.
...
-spec insert(hecate_dht_entry:entry(), siblings()) -> insert_result().
insert(Cand, S) ->
    insert(Cand, S, erlang:monotonic_time(millisecond)).

-spec insert(hecate_dht_entry:entry(), siblings(), integer()) -> insert_result().
insert(Cand, #{self_id := Self} = S, Now) ->
    Id = hecate_dht_entry:node_id(Cand),
    dispatch_insert(id_equals(Id, Self), Cand, Id, S, Now).

-spec dispatch_insert(boolean(), hecate_dht_entry:entry(),
                      hecate_dht_xor:id(), siblings(), integer()) ->
          insert_result().
...
-spec consider({ok, hecate_dht_entry:entry()} | error,
               hecate_dht_entry:entry(), siblings(), integer()) ->
          insert_result().
...
consider(error, Cand, #{self_id := Self, capacity := Cap, entries := Es} = S,
         _Now) ->
    Dist = hecate_dht_xor:distance_int(Self, hecate_dht_entry:node_id(Cand)),
    admit_or_replace(length(Es) < Cap, Dist, Cand, S).

-spec admit_or_replace(boolean(), non_neg_integer(),
                       hecate_dht_entry:entry(), siblings()) ->
          insert_result().
...
-spec decide_full(boolean(),
                  {non_neg_integer(), hecate_dht_entry:entry()},
                  non_neg_integer(), hecate_dht_entry:entry(),
...
-spec remove(hecate_dht_xor:id(), siblings()) -> siblings().
remove(<<_:256>> = Id, S) ->
    map_entries(fun(Es) -> drop_id(Id, Es) end, S).

-spec touch(hecate_dht_xor:id(), siblings(), integer()) -> siblings().
...
-spec id_equals(hecate_dht_xor:id(), hecate_dht_xor:id()) -> boolean().
id_equals(X, X) -> true;
id_equals(_, _) -> false.

-spec find_entry(hecate_dht_xor:id(), siblings()) ->
          {ok, hecate_dht_entry:entry()} | error.
find_entry(Id, #{entries := Es}) ->
    find_by_id(Id, Es).

-spec find_by_id(hecate_dht_xor:id(),
                 [{non_neg_integer(), hecate_dht_entry:entry()}]) ->
          {ok, hecate_dht_entry:entry()} | error.
...
-spec insert_sorted({non_neg_integer(), hecate_dht_entry:entry()},
                    [{non_neg_integer(), hecate_dht_entry:entry()}]) ->
          [{non_neg_integer(), hecate_dht_entry:entry()}].
insert_sorted(Pair, Es) ->
    lists:keymerge(1, [Pair], Es).

-spec drop_id(hecate_dht_xor:id(),
              [{non_neg_integer(), hecate_dht_entry:entry()}]) ->
          [{non_neg_integer(), hecate_dht_entry:entry()}].
drop_id(Id, Es) ->
    [Pair || {_, E} = Pair <- Es, hecate_dht_entry:node_id(E) =/= Id].

-spec touch_in(hecate_dht_xor:id(),
               [{non_neg_integer(), hecate_dht_entry:entry()}],
               integer()) ->
          [{non_neg_integer(), hecate_dht_entry:entry()}].
touch_in(Id, Es, Now) ->
...
-spec maybe_touch(hecate_dht_xor:id(),
                  {non_neg_integer(), hecate_dht_entry:entry()},
                  integer()) ->
          {non_neg_integer(), hecate_dht_entry:entry()}.
maybe_touch(Id, {D, E}, Now) ->
    apply_touch(hecate_dht_entry:node_id(E) =:= Id, D, E, Now).

-spec apply_touch(boolean(), non_neg_integer(),
                  hecate_dht_entry:entry(), integer()) ->
          {non_neg_integer(), hecate_dht_entry:entry()}.
apply_touch(true,  D, E, Now) -> {D, hecate_dht_entry:touch(E, Now)};
apply_touch(false, D, E, _Now) -> {D, E}.

-spec map_entries(fun(([{non_neg_integer(), hecate_dht_entry:entry()}]) ->
                          [{non_neg_integer(), hecate_dht_entry:entry()}]),
                  siblings()) -> siblings().
