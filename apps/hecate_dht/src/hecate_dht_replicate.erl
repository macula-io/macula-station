...
%% replicator as a worker child of `hecate_dht_sup' (wiring in a
...
replicate_record(Record, SelfId,
    ...
    Entries = hecate_dht:k_closest(Dht, Key, K),
    Targets = [hecate_dht_entry:node_id(E)
               || E <- Entries,
                  hecate_dht_entry:node_id(E) =/= SelfId],
...
send_one(PeerId, Record,
    ...
    Result = hecate_dht:send_store(Dht, PeerId, Record, Timeout),
    account(Result, Acc).

-spec account(hecate_dht:send_store_result(), outcome()) -> outcome().
