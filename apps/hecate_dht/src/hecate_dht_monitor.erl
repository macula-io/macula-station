...
%%   <li>Walks every populated bucket and asks `hecate_dht_diversity'
...
%%       runs `hecate_dht_placement:place/3' against the current
...
init(#{dht := Dht} = Opts) ->
    ...
    SelfId   = maps:get(self_id, Opts, hecate_dht:self_id(Dht)),
    State = #state{
        dht         = Dht,
        self_id     = SelfId,
...
build_report(#state{dht = Dht, self_id = SelfId} = State) ->
    Stats = hecate_dht:stats(Dht),
    #{
        self_id       => SelfId,
        rt_size       => maps:get(size, Stats),
        bucket_count  => maps:get(bucket_count, Stats),
        sibling_count => maps:get(sibling_count, Stats),
        record_count  => hecate_dht:record_count(Dht),
        buckets       => bucket_report(Dht),
        replicas      => replica_report(State)
    }.

-spec bucket_report(hecate_dht:dht()) -> bucket_report().
bucket_report(Dht) ->
    Buckets = hecate_dht:populated_buckets(Dht),
    Reports = [hecate_dht_diversity:bucket_constraints_met(
                 hecate_dht_bucket:members(B))
               || {_Ix, B} <- Buckets],
