...
%% `hecate_dht_routing_table' / `hecate_dht_server'.
%%
%% == Admission algorithm ==
%%
...
-type bucket() :: #{
    ...
    entries  := [hecate_dht_entry:entry()]
}.

-type insert_result() ::
      {admitted, bucket()}
    | {touched,  bucket()}
    | {replaced, Evicted :: hecate_dht_entry:entry(), bucket()}
    | {rejected, bucket()}.
...
-spec members(bucket()) -> [hecate_dht_entry:entry()].
members(#{entries := Es}) -> Es.

-spec find(hecate_dht_xor:id(), bucket()) ->
        {ok, hecate_dht_entry:entry()} | error.
find(Id, #{entries := Es}) ->
    find_by_id(Id, Es).

-spec contains(hecate_dht_xor:id(), bucket()) -> boolean().
contains(Id, Bucket) ->
...
-spec insert(hecate_dht_entry:entry(), bucket()) -> insert_result().
insert(Cand, Bucket) ->
    insert(Cand, Bucket, erlang:monotonic_time(millisecond)).

-spec insert(hecate_dht_entry:entry(), bucket(), integer()) -> insert_result().
insert(Cand, Bucket, Now) ->
    Id = hecate_dht_entry:node_id(Cand),
    dispatch_insert(find(Id, Bucket), Cand, Bucket, Now).

-spec dispatch_insert({ok, hecate_dht_entry:entry()} | error,
                      hecate_dht_entry:entry(), bucket(), integer()) ->
          insert_result().
dispatch_insert({ok, _Existing}, Cand, Bucket, Now) ->
    Id = hecate_dht_entry:node_id(Cand),
    {touched, replace(Id, fun(E) -> hecate_dht_entry:touch(E, Now) end, Bucket)};
dispatch_insert(error, Cand, Bucket, Now) ->
    insert_new(Cand, Bucket, Now).

-spec insert_new(hecate_dht_entry:entry(), bucket(), integer()) ->
          insert_result().
...
-spec maybe_evict(hecate_dht_entry:entry(),
                  {float(), hecate_dht_entry:entry()},
...
maybe_evict(Cand, {_MinScore, Weakest}, _CandScore, #{entries := Es} = B) ->
    WeakId = hecate_dht_entry:node_id(Weakest),
    Remaining = [E || E <- Es, hecate_dht_entry:node_id(E) =/= WeakId],
...
-spec remove(hecate_dht_xor:id(), bucket()) -> bucket().
remove(Id, #{entries := Es} = B) ->
    B#{entries := [E || E <- Es, hecate_dht_entry:node_id(E) =/= Id]}.

-spec touch(hecate_dht_xor:id(), bucket(), integer()) -> bucket().
touch(Id, Bucket, Now) ->
    replace(Id, fun(E) -> hecate_dht_entry:touch(E, Now) end, Bucket).
...
-spec score(hecate_dht_entry:entry(), [hecate_dht_entry:entry()], integer()) ->
          float().
score(E, Reference, Now) ->
    Others = [X || X <- Reference,
                   hecate_dht_entry:node_id(X) =/= hecate_dht_entry:node_id(E)],
    (?W_UPTIME     * hecate_dht_entry:uptime(E))
      + (?W_NOVELTY    * (hecate_dht_diversity:novelty_score(E, Others) / 3.0))
      + (?W_INCUMBENCY * incumbency(E, Now))
      - (?W_LATENCY    * latency_frac(hecate_dht_entry:latency(E))).

%%=====================================================================
%% Internal
%%=====================================================================

-spec find_by_id(hecate_dht_xor:id(), [hecate_dht_entry:entry()]) ->
          {ok, hecate_dht_entry:entry()} | error.
find_by_id(_Id, []) -> error;
find_by_id(Id, [E | Rest]) ->
    find_or_continue(hecate_dht_entry:node_id(E) =:= Id, E, Id, Rest).

-spec find_or_continue(boolean(), hecate_dht_entry:entry(),
                       hecate_dht_xor:id(), [hecate_dht_entry:entry()]) ->
          {ok, hecate_dht_entry:entry()} | error.
find_or_continue(true,  E, _Id, _Rest) -> {ok, E};
find_or_continue(false, _, Id, Rest)   -> find_by_id(Id, Rest).

-spec replace(hecate_dht_xor:id(),
              fun((hecate_dht_entry:entry()) -> hecate_dht_entry:entry()),
              bucket()) -> bucket().
replace(Id, F, #{entries := Es} = B) ->
    B#{entries := [maybe_replace(Id, F, E) || E <- Es]}.

-spec maybe_replace(hecate_dht_xor:id(),
                    fun((hecate_dht_entry:entry()) -> hecate_dht_entry:entry()),
                    hecate_dht_entry:entry()) -> hecate_dht_entry:entry().
maybe_replace(Id, F, E) ->
    apply_if_match(hecate_dht_entry:node_id(E) =:= Id, F, E).
...
-spec weakest([hecate_dht_entry:entry(), ...], integer()) ->
          {float(), hecate_dht_entry:entry()}.
weakest(Es, Now) ->
    Scored = [{score(E, Es, Now), E} || E <- Es],
    lists:foldl(fun pick_min/2, hd(Scored), tl(Scored)).

-spec pick_min({float(), hecate_dht_entry:entry()},
               {float(), hecate_dht_entry:entry()}) ->
          {float(), hecate_dht_entry:entry()}.
pick_min({S, _} = A, {Best, _} = _B) when S < Best -> A;
pick_min(_A, B) -> B.

-spec incumbency(hecate_dht_entry:entry(), integer()) -> float().
incumbency(E, Now) ->
    Age = hecate_dht_entry:time_in_table_ms(E, Now),
