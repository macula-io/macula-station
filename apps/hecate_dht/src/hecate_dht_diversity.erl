...
%%       (used by `hecate_dht_bucket' when ranking admit vs. evict).</li>
%% </ul>
%%
%% This module owns *no* state and *no* thresholds for replica placement
%% — that is `hecate_dht_placement' (Session 3.8).
%%
%% Reference: plans/PLAN_MACULA_V2_PART3_DISCOVERY.md §4.3.
-module(hecate_dht_diversity).
...
-spec distinct_asns([hecate_dht_entry:entry()]) -> non_neg_integer().
distinct_asns(Entries) ->
    distinct(fun hecate_dht_entry:asn/1, Entries).

-spec distinct_countries([hecate_dht_entry:entry()]) -> non_neg_integer().
distinct_countries(Entries) ->
    distinct(fun hecate_dht_entry:country/1, Entries).

-spec distinct_tiers([hecate_dht_entry:entry()]) -> non_neg_integer().
distinct_tiers(Entries) ->
    distinct(fun hecate_dht_entry:tier/1, Entries).

-spec counts([hecate_dht_entry:entry()]) -> counts().
counts(Entries) ->
    #{
...
-spec bucket_constraints_met([hecate_dht_entry:entry()]) -> report().
bucket_constraints_met(Entries) ->
    bucket_constraints_met(Entries, soft_constraints()).

-spec bucket_constraints_met([hecate_dht_entry:entry()], constraints()) -> report().
...
-spec novelty_score(hecate_dht_entry:entry(), [hecate_dht_entry:entry()]) -> float().
novelty_score(Candidate, Entries) ->
    Asns      = set_of(fun hecate_dht_entry:asn/1,     Entries),
    Countries = set_of(fun hecate_dht_entry:country/1, Entries),
    Tiers     = set_of(fun hecate_dht_entry:tier/1,    Entries),
    bonus(hecate_dht_entry:asn(Candidate),     Asns,      ?W_ASN)
      + bonus(hecate_dht_entry:country(Candidate), Countries, ?W_COUNTRY)
      + bonus(hecate_dht_entry:tier(Candidate),    Tiers,     ?W_TIER).

%%=====================================================================
%% Internal
%%=====================================================================

-spec distinct(fun((hecate_dht_entry:entry()) -> term()),
               [hecate_dht_entry:entry()]) -> non_neg_integer().
distinct(F, Entries) ->
    sets:size(set_of(F, Entries)).

-spec set_of(fun((hecate_dht_entry:entry()) -> T),
             [hecate_dht_entry:entry()]) -> sets:set(T).
set_of(F, Entries) ->
