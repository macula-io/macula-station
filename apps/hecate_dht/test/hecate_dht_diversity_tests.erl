%% EUnit tests for hecate_dht_diversity.
-module(hecate_dht_diversity_tests).
...
distinct_counts_unique_entries_test() ->
    ...
    ?assertEqual(#{asns => 3, countries => 3, tiers => 3},
                 hecate_dht_diversity:counts(Entries)).
...
empty_bucket_tier_exempt_test() ->
    #{tier_ok := Ok} = hecate_dht_diversity:bucket_constraints_met([]),
    ?assert(Ok).

small_bucket_tier_exempt_test() ->
    Entries = [entry(N, 100, <<"BE">>, t0) || N <- lists:seq(1, 7)],
    #{tier_ok := Ok} = hecate_dht_diversity:bucket_constraints_met(Entries),
    ?assert(Ok).
...
large_bucket_two_tiers_passes_test() ->
    ...
    Report = hecate_dht_diversity:bucket_constraints_met(Entries),
...
custom_constraints_honoured_test() ->
    ...
    ?assertEqual(#{asn_ok => true, country_ok => true, tier_ok => true},
                 hecate_dht_diversity:bucket_constraints_met(Entries, Custom)).
