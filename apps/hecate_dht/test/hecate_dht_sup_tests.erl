%% EUnit tests for hecate_dht_sup.
-module(hecate_dht_sup_tests).

-include_lib("eunit/include/eunit.hrl").

start_link_returns_sup_pid_test() ->
    {ok, Sup} = hecate_dht_sup:start_link(#{self_id => <<0:256>>}),
    ?assert(is_pid(Sup)),
    unlink_and_stop(Sup).
...
sup_starts_server_child_test() ->
    ...
    ?assertEqual(<<0:256>>, hecate_dht:self_id(Server)),
    unlink_and_stop(Sup).

sup_propagates_opts_to_server_test() ->
    {ok, Sup} = hecate_dht_sup:start_link(
                  #{self_id => <<7:256>>, k => 5, s => 3}),
    {ok, Server} = hecate_dht_sup:get_server(Sup),
    ?assertMatch(#{self_id := <<7:256>>, k := 5, s := 3},
                 hecate_dht:stats(Server)),
    unlink_and_stop(Sup).

sup_restarts_crashed_server_test() ->
    {ok, Sup} = hecate_dht_sup:start_link(#{self_id => <<0:256>>}),
    {ok, S1} = hecate_dht_sup:get_server(Sup),
    ...
    timer:sleep(50),
    {ok, S2} = hecate_dht_sup:get_server(Sup),
