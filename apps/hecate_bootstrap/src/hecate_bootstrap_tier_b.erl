...
%%       `hecate_bootstrap_mdns_transport'. Defaults to
%%       `hecate_bootstrap_mdns_udp' (real multicast UDP).</li>
...
-module(hecate_bootstrap_tier_b).
...
probe(Opts) ->
    UdpMod     = maps:get(udp_transport, Opts, hecate_bootstrap_mdns_udp),
...
run(UdpMod, Handshake, Timeout) ->
    {_Id, Query} = hecate_bootstrap_mdns:build_query(),
...
answers_to_candidates(Src, Bin) ->
    case hecate_bootstrap_mdns:parse_response(Bin) of
        {ok, Answers} ->
            [C#{src_addr => Src}
