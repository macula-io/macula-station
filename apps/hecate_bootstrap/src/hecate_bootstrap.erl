...
%% of tier probe modules (implementing `hecate_bootstrap_tier') and
...
%% ```
%% Tiers = [
%%   {hecate_bootstrap_tier_e, #{peer_urls => [Url1, Url2, Url3]}}
%% ],
%% {ok, Peers} = hecate_bootstrap:cascade(Tiers, #{min_peers => 3}).
%% '''
%%
%% Peers are `hecate_bootstrap_tier:verified_peer()' maps — signature
%% and expiry have already been checked by the yielding tier. The
%% caller feeds them into the DHT routing table (`hecate_dht:observe/2')
