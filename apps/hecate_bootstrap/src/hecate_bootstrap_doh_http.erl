...
%%
%% Thin behaviour-conforming wrapper around `hecate_bootstrap_doh':
...
%%
%% <ul>
%%   <li>`application:get_env(hecate_bootstrap, doh_zone_base, _)' —
...
%% They are declared as application dependencies of `hecate_bootstrap'.
%%
%% This module has no unit tests — its single responsibility is
%% translating `{ok, HttpStatus, Body}' into the shape
%% `hecate_bootstrap_doh:resolve/4' expects. Coverage lives in
%% `hecate_bootstrap_doh_tests' (codec) and in the network-integrated
%% CT suite gated on `MACULA_DOH_ENABLE=1'.
-module(hecate_bootstrap_doh_http).
-behaviour(hecate_bootstrap_resolver).
...
resolve(Url, Pubkey, Opts) ->
    ...
    ZoneBase = application:get_env(hecate_bootstrap, doh_zone_base,
                                   <<"macula.io">>),
    CodecOpts = #{zone_base => ZoneBase, timeout_ms => Timeout},
    hecate_bootstrap_doh:resolve(Url, Pubkey, CodecOpts,
                                 send_fun(Timeout)).
