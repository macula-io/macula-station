%% @doc `inets:httpc'-backed `hecate_bootstrap_http' implementation.
%%
...
%% adapters.
-module(hecate_bootstrap_http_httpc).
-behaviour(hecate_bootstrap_http).

-export([get/2, post_json/3]).

-define(CT_JSON, "application/json").

-spec get(hecate_bootstrap_http:url(), pos_integer()) ->
          hecate_bootstrap_http:get_result().
get(Url, TimeoutMs) ->
...
-spec post_json(hecate_bootstrap_http:url(), binary(), pos_integer()) ->
          hecate_bootstrap_http:post_result().
