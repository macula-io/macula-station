-module(hecate_bootstrap_chain_esplora_tests).
...
parse_op_return_raw_push_test() ->
    ...
    {ok, #{hash := H, url := U}} =
        hecate_bootstrap_chain_esplora:parse_op_return(Script),
    ?assertEqual(Hash, H),
...
parse_op_return_non_op_return_test() ->
    ...
    ?assertEqual({error, bad_op_return_script},
                 hecate_bootstrap_chain_esplora:parse_op_return(
                   <<16#00, 1, 2, 3>>)).
...
extract_anchor_skips_non_op_return_vouts_test() ->
    ...
    ?assertMatch({ok, #{hash := _, url := _}},
                 hecate_bootstrap_chain_esplora:extract_anchor_pointer(Tx)).

extract_anchor_missing_vout_test() ->
    ?assertEqual({error, bad_tx},
                 hecate_bootstrap_chain_esplora:extract_anchor_pointer(
                   #{<<"txid">> => <<"abc">>})).
...
happy_end_to_end(_Ctx) ->
    fun() ->
        ...
        hecate_bootstrap_http_fake:set_get(
          ?TXS_URL, {ok, iolist_to_binary(json:encode(Txs))}),
        hecate_bootstrap_http_fake:set_get(?URL, {ok, Bytes}),
...
hash_mismatch_rejected(_Ctx) ->
    fun() ->
        ...
        hecate_bootstrap_http_fake:set_get(
          ?TXS_URL, {ok, iolist_to_binary(json:encode(Txs))}),
        hecate_bootstrap_http_fake:set_get(?URL, {ok, Actual}),
...
url_fetch_error(_Ctx) ->
    fun() ->
        ...
        hecate_bootstrap_http_fake:set_get(
          ?TXS_URL, {ok, iolist_to_binary(json:encode(Txs))}),
        hecate_bootstrap_http_fake:set_get(?URL, {error, timeout}),
...
probe() ->
    hecate_bootstrap_chain_esplora:latest_anchor(
      #{base_url => ?BASE,
        address  => ?ADDR,
