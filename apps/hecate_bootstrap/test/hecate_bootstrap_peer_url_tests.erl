%% EUnit tests for hecate_bootstrap_peer_url.
-module(hecate_bootstrap_peer_url_tests).
...
roundtrip_with_address_hints_test() ->
    ...
    {ok, _Decoded, Addrs} = hecate_bootstrap_peer_url:decode(Url),
...
decode_bad_scheme_test() ->
    ?assertEqual({error, bad_scheme},
                 hecate_bootstrap_peer_url:decode(<<"http://nope">>)).

decode_malformed_base64_test() ->
    ?assertEqual({error, bad_base64},
                 hecate_bootstrap_peer_url:decode(
...
decode_non_cbor_payload_test() ->
    ...
    ?assertMatch({error, _}, hecate_bootstrap_peer_url:decode(Url)).
    ...
    Endorsement = macula_record:sign(
        macula_record:realm_member_endorsement(
          RealmId, #{realm => RealmId, member_node => Member,
                     roles => [<<"member">>]}),
        AdminKp),
    Url = hecate_bootstrap_peer_url:encode(Endorsement, []),
    ?assertEqual({error, wrong_type},
                 hecate_bootstrap_peer_url:decode(Url)).
...
decode_rejects_tampered_record_test() ->
    ...
    Result = hecate_bootstrap_peer_url:decode(Flipped),
