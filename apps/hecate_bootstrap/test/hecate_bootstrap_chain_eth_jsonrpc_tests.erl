-module(hecate_bootstrap_chain_eth_jsonrpc_tests).
...
build_request_shape_test() ->
    Req = hecate_bootstrap_chain_eth_jsonrpc:build_request(
            ?CONTRACT, ?TOPIC,
...
parse_response_no_logs_test() ->
    ...
    ?assertEqual({error, no_logs},
                 hecate_bootstrap_chain_eth_jsonrpc:parse_response(Body)).
...
parse_response_bad_json_test() ->
    ?assertEqual({error, bad_json},
                 hecate_bootstrap_chain_eth_jsonrpc:parse_response(
                   <<"not json">>)).
...
decode_bytes_arg_happy_test() ->
    ...
    ?assertEqual({ok, Payload},
                 hecate_bootstrap_chain_eth_jsonrpc:decode_bytes_arg(Abi)).

decode_bytes_arg_zero_length_rejected_test() ->
    ?assertEqual({error, bad_abi},
                 hecate_bootstrap_chain_eth_jsonrpc:decode_bytes_arg(
                   <<0:256, 0:256>>)).
...
decode_bytes_arg_length_exceeds_payload_test() ->
    ...
    ?assertEqual({error, bad_abi},
                 hecate_bootstrap_chain_eth_jsonrpc:decode_bytes_arg(Bin)).
    ...
    {setup,
     ...
     fun(_Ctx) ->
        fun() ->
            ...
            Log = mk_log(<<"0x1">>, Payload),
            Body = iolist_to_binary(json:encode(rpc_ok([Log]))),
            hecate_bootstrap_http_fake:set_post(?ENDPOINT, {ok, Body}),
            Opts = #{endpoint => ?ENDPOINT,
                     contract => ?CONTRACT,
                     topic    => ?TOPIC,
                     http     => hecate_bootstrap_http_fake},
            ?assertEqual({ok, Payload},
                         hecate_bootstrap_chain_eth_jsonrpc:latest_anchor(
                           Opts, 500))
        end
     end}.
...
latest_anchor_http_error_wrapped_test_() ->
    {setup,
     ...
     fun(_Ctx) ->
        fun() ->
            hecate_bootstrap_http_fake:set_post(
