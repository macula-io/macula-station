-module(hecate_bootstrap_tier_b_tests).
...
happy_path(#{peers := Peers, handshake := Handshake}) ->
    fun() ->
        ...
        [?assertEqual(hecate_bootstrap_tier_b, maps:get(via, G))
         || G <- Got]
    end.

no_replies(#{handshake := Handshake}) ->
    fun() ->
        hecate_bootstrap_mdns_fake:set_replies([]),
        ?assertEqual({ok, []}, probe(Handshake))
    end.

handshake_failure_drops_candidate(#{peers := [P1 | _]}) ->
    fun() ->
        hecate_bootstrap_mdns_fake:set_replies([peer_to_reply(P1)]),
    ...
    fun() ->
        ...
        Bad = {addr(99), build_response(
                          [{"_macula._udp.local", txt,
                            ["broken_txt"]}])},
        hecate_bootstrap_mdns_fake:set_replies([Bad, Good]),
        {ok, Got} = probe(Handshake),
...
deduplicates_by_node_id(#{peers := [P1 | _], handshake := Handshake}) ->
    fun() ->
        ...
        hecate_bootstrap_mdns_fake:set_replies([Reply1, Reply2]),
        {ok, Got} = probe(Handshake),
        ?assertEqual(1, length(Got))
    end.
...
missing_handshake_opt_drops_candidate(#{peers := [P1 | _]}) ->
    fun() ->
        hecate_bootstrap_mdns_fake:set_replies([peer_to_reply(P1)]),
        {ok, Got} = hecate_bootstrap_tier_b:probe(
