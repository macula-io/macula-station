...
%% `hecate_bootstrap_doh:resolve/4' codec over a fake HTTP transport
...
doh_end_to_end(#{fk := Fk}) ->
    fun() ->
        Resolvers = [{?MODULE, ?URL1},
                     {?MODULE, ?URL2},
                     {?MODULE, ?URL3}],
        {ok, Peers} = hecate_bootstrap_tier_a:probe(
                        #{resolvers     => Resolvers,
                          pubkeys       => [Fk],
                          corroboration => 2,
                          timeout_ms    => 1_000}),
        ...
        ?assert(lists:all(fun(#{tier := T, via := V}) ->
                                  T =:= a andalso V =:= hecate_bootstrap_tier_a
                          end, Peers))
    end.
...
resolve(Url, Pubkey, Opts) ->
    ...
    hecate_bootstrap_doh:resolve(Url, Pubkey, Opts, Send).
