...
tier_e_below_min_peers_fails_test() ->
    ...
    Tiers = [{hecate_bootstrap_tier_e, #{peer_urls => [url()]}}],
    ?assertEqual({error, cascade_failed},
                 hecate_bootstrap:cascade(Tiers,
                     #{min_peers => 3, timeout_ms => 500})).

tier_e_no_urls_fails_test() ->
    Tiers = [{hecate_bootstrap_tier_e, #{}}],
    ?assertEqual({error, cascade_failed},
                 hecate_bootstrap:cascade(Tiers,
                     #{min_peers => 3, timeout_ms => 500})).
...
fast_probe_preempts_slow_probe_test() ->
    ...
    try
        Tiers = [{slow_tier, #{}},
                 {hecate_bootstrap_tier_e, #{peer_urls => FastUrls}}],
...
failing_probes_fall_through_to_working_probe_test() ->
    ...
    try
        ...
        {ok, Peers} = hecate_bootstrap:cascade(Tiers,
