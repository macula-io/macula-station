%% @doc Fan-out tests for `hecate_bootstrap_mdns_udp:query/3'.
%%
...
-module(hecate_bootstrap_mdns_udp_tests).
...
single_socket_path_returns_empty_on_open_failure_test() ->
    ...
    ?assertEqual([],
                 hecate_bootstrap_mdns_udp:query(
                   <<"q">>, 50,
...
fan_out_merges_per_interface_replies_test() ->
    ...
    Ifaces = [#{name => "iA", index => 10,
              ...
              #{name => "iB", index => 11,
                link_local => undefined, ipv6 => []}],
    Replies = hecate_bootstrap_mdns_udp:query(
                <<"probe">>, 500,
                #{socket_opener => Opener,
                  interfaces    => Ifaces}),
...
%% `hecate_bootstrap_mdns:multicast_group()' on port 5353. We can't
