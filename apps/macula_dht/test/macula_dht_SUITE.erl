...
fleet_lifecycle_loops_observed(_Config) ->
    ...
    ok = macula_dht:put_record(Dht, OwnedRecord),
    Stale = macula_record:sign(
              macula_record:node_record(macula_identity:public(
                                          macula_identity:generate()),
                                        [], 0, #{ttl_ms => 1}),
              Kp),
    ok = macula_dht:put_record(Dht, Stale),
    timer:sleep(5),

    {ok, Replicate} = macula_dht_replicate:start_link(
                        #{dht => Dht, interval_ms => 100,
                          per_store_timeout_ms => 50}),
    {ok, Republish} = macula_dht_republish:start_link(
                        #{dht => Dht, identity => Kp, interval_ms => 100,
                          store_opts => #{store_timeout_ms => 50,
                                          overall_timeout_ms => 100}}),
    {ok, Expire} = macula_dht_expire:start_link(
                     #{dht => Dht, interval_ms => 100}),

    timer:sleep(600),

    #{ticks := RT}  = macula_dht_replicate:stats(Replicate),
    #{ticks := PT}  = macula_dht_republish:stats(Republish),
    #{ticks := ET}  = macula_dht_expire:stats(Expire),
    ...
    macula_dht_replicate:stop(Replicate),
    macula_dht_republish:stop(Republish),
    macula_dht_expire:stop(Expire),
    macula_dht:stop(Dht).
...
fleet_round_trip_pings_succeed(Config) ->
    ...
    Results = [macula_dht:ping_peer(maps:get(pid, Src),
...
teardown_fleet(#{router := Router, stations := Stations}) ->
    [macula_dht:stop(maps:get(pid, S)) || S <- Stations],
    Router ! stop,
    ok.
...
bootstrap_one(Station, AllStations, K) ->
    ...
    lists:foreach(
      fun(Other) ->
          admitted = macula_dht:observe(Pid, peer_spec(Other))
      end, Sampled).
...
run_lookup(Src, TargetKey) ->
    macula_dht:lookup_nodes(maps:get(pid, Src), TargetKey,
...
station_bucket_report(Station) ->
    ...
    SelfId = macula_identity:public(Kp),
    {ok, Mon} = macula_dht_monitor:start_link(
                  #{dht => Pid, interval_ms => 3_600_000,
                    self_id => SelfId}),
    try
        maps:get(buckets, macula_dht_monitor:report(Mon))
    after
        macula_dht_monitor:stop(Mon)
    end.
