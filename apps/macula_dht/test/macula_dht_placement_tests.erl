%% EUnit tests for macula_dht_placement — the pure greedy algorithm.
-module(macula_dht_placement_tests).
...
default_constraints_have_expected_fields_test() ->
    C = macula_dht_placement:default_constraints(),
...
empty_candidate_pool_returns_empty_chosen_test() ->
    R = macula_dht_placement:place(<<0:256>>, []),
...
chosen_ordered_by_distance_and_capped_at_k_test() ->
    ...
    R = macula_dht_placement:place(Key, Refs, small_constraints(5)),
    Chosen = maps:get(chosen, R),
    ?assertEqual(5, length(Chosen)),
    Ids = [maps:get(node_id, X) || X <- Chosen],
    Distances = [macula_dht_xor:distance_int(Key, Id) || Id <- Ids],
