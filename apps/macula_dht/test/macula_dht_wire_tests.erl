%% End-to-end wire tests for macula_dht Session 3.5 — PING/PONG and
...
ping_roundtrip_marks_target_last_seen_test() ->
    ...
    {ok, BEntryBefore} = macula_dht:find(A, BId),
    timer:sleep(5),

    {ok, #{rtt_ms := Rtt}} = macula_dht:ping_peer(A, BId),
    ?assert(Rtt >= 0),

    {ok, BEntryAfter} = macula_dht:find(A, BId),
    ?assert(macula_dht_entry:last_seen(BEntryAfter)
              >= macula_dht_entry:last_seen(BEntryBefore)),
...
ping_timeout_when_peer_does_not_reply_test() ->
    ...
    ?assertEqual({error, timeout},
                 macula_dht:ping_peer(A, <<7:256>>, 120)),
    macula_dht:stop(A).
...
pong_with_wrong_nonce_is_ignored_test() ->
    ...
    Spoof = macula_frame:sign(
              ...
              Kp),
    ok = macula_dht:handle_frame(D, SelfId, Spoof),
    %% Sanity call to sync: server must still respond.
    ?assertEqual(0, macula_dht:size(D)),
    macula_dht:stop(D).
...
tampered_frame_is_dropped_silently_test() ->
    ...
    Tamper = fun(_DstId, Frame) ->
        ...
        macula_dht:handle_frame(A, BId, Mutated)
    end,
    {ok, B} = macula_dht:start_link(#{
        self_id         => BId,
        identity        => KpB,
        send_frame      => Tamper,
        ping_timeout_ms => 120
    ...
    ?assertEqual({error, timeout},
                 macula_dht:ping_peer(B, AId, 120)),
    macula_dht:stop(A),
    macula_dht:stop(B).
...
find_node_returns_peers_target_knows_test() ->
    ...
    Peer2Id = <<2:8, 0:248>>,
    Peer3Id = <<3:8, 0:248>>,
    admitted = macula_dht:observe(B, spec(Peer1Id)),
    admitted = macula_dht:observe(B, spec(Peer2Id)),
    admitted = macula_dht:observe(B, spec(Peer3Id)),

    {ok, Refs} = macula_dht:find_node(A, <<1:8, 0:248>>, BId),
...
find_node_returns_empty_when_target_has_no_peers_test() ->
    #{a := {A, KpA}, b := {B, KpB}} = start_pair(),
    BId = macula_identity:public(KpB),
    {ok, []} = macula_dht:find_node(A, <<9:8, 0:248>>, BId),
    stop_pair(#{a => {A, KpA}, b => {B, KpB}}).
...
find_node_timeout_when_peer_is_silent_test() ->
    ...
    ?assertEqual({error, timeout},
                 macula_dht:find_node(D, <<1:256>>, <<2:256>>, 120)),
    macula_dht:stop(D).
...
find_node_no_transport_returns_error_test() ->
    ...
    {ok, D} = macula_dht:start_link(#{self_id => macula_identity:public(Kp)}),
    ?assertEqual({error, no_transport},
                 macula_dht:find_node(D, <<1:256>>, <<2:256>>)),
    macula_dht:stop(D).
...
concurrent_pings_to_different_peers_test() ->
    ...
    {ok, A} = macula_dht:start_link(#{self_id => AId, identity => KpA,
                                      send_frame => SendFrom(AId)}),
    {ok, B} = macula_dht:start_link(#{self_id => BId, identity => KpB,
                                      send_frame => SendFrom(BId)}),
    {ok, C} = macula_dht:start_link(#{self_id => CId, identity => KpC,
                                      send_frame => SendFrom(CId)}),
    register_router(Router, #{AId => A, BId => B, CId => C}),

    Parent = self(),
    spawn_link(fun() ->
        Parent ! {r1, macula_dht:ping_peer(A, BId, 2_000)}
    end),
    spawn_link(fun() ->
        Parent ! {r2, macula_dht:ping_peer(A, CId, 2_000)}
    end),
            ...
            case Table of
                ...
                _ ->
                    case maps:find(DstId, Table) of
                        {ok, Pid} -> macula_dht:handle_frame(Pid, FromId, F);
                        error     -> ok
                    end,
                    router_loop(Table)
            end;
