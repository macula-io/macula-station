...
-module(macula_dht_lifecycle_tests).
...
republish_empty_store_is_noop_test() ->
    ...
    Outcome = macula_dht_republish:tick(R),
...
republish_refreshes_owned_record_version_test() ->
    ...
    [Refreshed] = macula_dht:find_local_record(A, AId),
...
republish_propagates_to_custodian_test() ->
    ...
    [OnB] = macula_dht:find_local_record(B, AId),
...
expire_empty_store_is_noop_test() ->
    ...
    #{a := {A, _}} = Net,
    R = start_expire(A),
    Outcome = macula_dht_expire:tick(R),
    ?assertEqual(0, maps:get(records_seen, Outcome)),
...
expire_retains_fresh_record_test() ->
    ...
    ?assertEqual(1, macula_dht:record_count(A)),
    stop_expire(R),
    stop_net(Net).
...
expire_reaps_past_ttl_record_test() ->
    ...
    ?assertEqual(0, macula_dht:record_count(A)),
    stop_expire(R),
    stop_net(Net).
...
expire_separates_fresh_from_stale_records_test() ->
    ...
    ok = macula_dht:put_record(A, LongLived),
    ok = macula_dht:put_record(A, Ephemeral),
    timer:sleep(5),
    ...
    ?assertEqual(1, maps:get(kept, Outcome)),
    ?assertEqual(1, macula_dht:record_count(A)),
    [Kept] = macula_dht:find_local_record(
...
expire_timer_fires_on_interval_test() ->
    ...
    timer:sleep(180),
    #{ticks := Ticks} = macula_dht_expire:stats(R),
    ?assert(Ticks >= 2),
    stop_expire(R),
    stop_net(Net).
...
start_republisher(Dht, Identity, Extra) ->
    ...
    {ok, R} = macula_dht_republish:start_link(maps:merge(Base, Extra)),
    R.

stop_republisher(R) -> macula_dht_republish:stop(R).
...
start_expire(Dht, Extra) ->
    ...
    {ok, R} = macula_dht_expire:start_link(maps:merge(Base, Extra)),
    R.

stop_expire(R) -> macula_dht_expire:stop(R).
...
make_server(Router) ->
    ...
    {ok, P} = macula_dht:start_link(#{self_id    => SelfId,
                                      identity   => Kp,
                                      send_frame => Send}),
    {P, Kp}.
