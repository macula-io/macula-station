...
-module(macula_dht_find_value_tests).
...
find_value_hit_returns_value_from_peer_test() ->
    ...
    Rec = macula_record:sign(
            macula_record:realm_directory(RealmId, <<"test realm">>,
                                          RealmId),
            RealmKp),
    ok = macula_dht:put_record(B, Rec),

    Result = macula_dht:find_value(A, RealmId, BId),
...
find_value_miss_falls_back_to_nodes_test() ->
    ...
    admitted = macula_dht:observe(B, peer_spec(<<1:256>>)),
    admitted = macula_dht:observe(B, peer_spec(<<2:256>>)),

    Result = macula_dht:find_value(A, <<99:256>>, BId),
...
find_value_returns_all_records_at_storage_key_test() ->
    ...
    ok = macula_dht:put_record(B, Rec1),
    ok = macula_dht:put_record(B, Rec2),

    {value, Records} = macula_dht:find_value(A, UriHash, BId),
...
find_value_timeout_on_silent_peer_test() ->
    ...
    ?assertEqual({error, timeout},
                 macula_dht:find_value(A, <<1:256>>, <<2:256>>, 120)),
    macula_dht:stop(A).
...
find_value_no_transport_returns_error_test() ->
    ...
    ?assertEqual({error, no_transport},
                 macula_dht:find_value(D, <<1:256>>, <<2:256>>)),
    macula_dht:stop(D).
