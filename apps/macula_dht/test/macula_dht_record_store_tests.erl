...
%% macula_dht:put_record/2, find_local_record/2, record_count/1.
-module(macula_dht_record_store_tests).
...
put_node_record_indexed_by_node_id_test() ->
    ...
    ok = macula_dht:put_record(D, R),
    ?assertEqual([R], macula_dht:find_local_record(D, NodeId)),
    ?assertEqual(1, macula_dht:record_count(D)),
    macula_dht:stop(D).
...
put_realm_stations_indexed_by_hashed_station_set_test() ->
    ...
    ?assertEqual([R], macula_dht:find_local_record(D, HashedKey)),
    macula_dht:stop(D).
...
two_advertisers_same_procedure_both_stored_test() ->
    ...
    ok = macula_dht:put_record(D, Ra),
    ok = macula_dht:put_record(D, Rb),
    Stored = macula_dht:find_local_record(D, crypto:hash(sha256, Uri)),
...
put_record_twice_same_owner_replaces_test() ->
    ...
    R1 = macula_record:sign(
           macula_record:node_record(NodeId, [], 0), OwnerKp),
    ok = macula_dht:put_record(D, R1),
    timer:sleep(2),
    R2 = macula_record:sign(
           macula_record:node_record(NodeId, [], 1), OwnerKp),
    ok = macula_dht:put_record(D, R2),
    ?assertEqual([R2], macula_dht:find_local_record(D, NodeId)),
    ?assertEqual(1, macula_dht:record_count(D)),
    macula_dht:stop(D).
...
find_local_record_missing_returns_empty_list_test() ->
    {D, _} = start(),
    ?assertEqual([], macula_dht:find_local_record(D, <<0:256>>)),
    macula_dht:stop(D).

fresh_server_record_count_is_zero_test() ->
    {D, _} = start(),
    ?assertEqual(0, macula_dht:record_count(D)),
    macula_dht:stop(D).
...
on_record_stored_callback_fires_for_put_record_test() ->
    ...
    Record = macula_record:sign(
               macula_record:node_record(NodeId, [], 0), OwnerKp),
    ok = macula_dht:put_record(D, Record),
    receive
...
on_record_stored_callback_can_be_cleared_test() ->
    ...
    ok = macula_dht:set_on_record_stored(D, undefined),
    Record = macula_record:sign(
               macula_record:node_record(
                 macula_identity:public(OwnerKp), [], 0), OwnerKp),
    ok = macula_dht:put_record(D, Record),
