...
%%   <li><strong>Placement</strong> — `macula_dht_placement:place/3'
...
%%       chosen peer calling `macula_dht:send_store/4'. Collects
...
-type opts() :: #{
    ...
    constraints         => macula_dht_placement:constraints(),
...
store(Dht, Record, Opts) when is_map(Record), is_map(Opts) ->
    ...
    Placement   = macula_dht_placement:place(Key, Candidates, Constraints),
...
-spec resolve_constraints(opts(), pos_integer()) ->
          macula_dht_placement:constraints().
resolve_constraints(Opts, K) ->
    Default = macula_dht_placement:default_constraints(),
    Custom  = maps:get(constraints, Opts, #{}),
    maps:merge(Default#{k := K}, Custom).

-spec resolve_candidates(macula_dht:dht(), macula_dht_xor:id(),
                         pos_integer(), opts()) ->
...
-spec pick_candidates({ok, [macula_frame:station_ref()]} | error,
                      macula_dht:dht(), macula_dht_xor:id(),
                      pos_integer(), opts()) ->
          [macula_frame:station_ref()].
...
pick_candidates(error, Dht, Key, K, Opts) ->
    ...
    Entries = macula_dht:k_closest(Dht, Key, K * Factor),
    [macula_dht_protocol:entry_to_station_ref(E) || E <- Entries].
