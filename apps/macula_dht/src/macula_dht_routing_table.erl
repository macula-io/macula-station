...
%% `macula_dht_xor:bucket_index(Self, Other) == i', i.e. peers that
...
%% `macula_dht_server'. Buckets below `macula_dht_bucket' keep all
...
-module(macula_dht_routing_table).
...
-type table() :: #{
    ...
    buckets  := #{macula_dht_xor:bucket_ix() => macula_dht_bucket:bucket()}
}.

-type insert_result() ::
      {admitted, table()}
    | {touched,  table()}
    | {replaced, Evicted :: macula_dht_entry:entry(), table()}
    | {rejected, table()}.
...
-spec new(macula_dht_xor:id()) -> table().
new(Self) ->
    new(Self, ?DEFAULT_K).

-spec new(macula_dht_xor:id(), pos_integer()) -> table().
...
-spec self_id(table())  -> macula_dht_xor:id().
self_id(#{self_id := V})  -> V.
...
size(#{buckets := Bs}) ->
    maps:fold(fun(_, B, Acc) -> Acc + macula_dht_bucket:size(B) end, 0, Bs).

-spec bucket_count(table()) -> non_neg_integer().
bucket_count(#{buckets := Bs}) -> map_size(Bs).

-spec bucket_at(macula_dht_xor:bucket_ix(), table()) -> macula_dht_bucket:bucket().
bucket_at(Ix, #{capacity := K, buckets := Bs}) ->
    maps:get(Ix, Bs, macula_dht_bucket:new(K)).
...
-spec find(macula_dht_xor:id(), table()) ->
        {ok, macula_dht_entry:entry()} | error.
find(Id, #{self_id := Self} = T) ->
    find_at(macula_dht_xor:bucket_index(Self, Id), Id, T).

-spec contains(macula_dht_xor:id(), table()) -> boolean().
contains(Id, T) ->
    case find(Id, T) of
        {ok, _} -> true;
        error   -> false
...
-spec insert(macula_dht_entry:entry(), table()) -> insert_result().
insert(Cand, T) ->
    insert(Cand, T, erlang:monotonic_time(millisecond)).

-spec insert(macula_dht_entry:entry(), table(), integer()) -> insert_result().
insert(Cand, #{self_id := Self} = T, Now) ->
    Id = macula_dht_entry:node_id(Cand),
    dispatch_insert(macula_dht_xor:bucket_index(Self, Id), Cand, T, Now).

-spec dispatch_insert(macula_dht_xor:bucket_ix(),
                      macula_dht_entry:entry(), table(), integer()) ->
          insert_result().
dispatch_insert(-1, _Cand, T, _Now) ->
    {rejected, T};
dispatch_insert(Ix, Cand, T, Now) ->
    Bucket = bucket_at(Ix, T),
    relay_bucket_result(macula_dht_bucket:insert(Cand, Bucket, Now), Ix, T).

-spec relay_bucket_result(macula_dht_bucket:insert_result(),
                          macula_dht_xor:bucket_ix(), table()) ->
          insert_result().
...
-spec remove(macula_dht_xor:id(), table()) -> table().
remove(<<_:256>> = Id, #{self_id := Self} = T) ->
    remove_at(macula_dht_xor:bucket_index(Self, Id), Id, T).

-spec remove_at(macula_dht_xor:bucket_ix(), macula_dht_xor:id(), table()) ->
          table().
remove_at(-1, _Id, T) -> T;
remove_at(Ix, Id, #{buckets := Bs} = T) ->
    update_existing(maps:find(Ix, Bs), Ix, Id, T).

-spec update_existing({ok, macula_dht_bucket:bucket()} | error,
                      macula_dht_xor:bucket_ix(), macula_dht_xor:id(),
                      table()) -> table().
update_existing(error, _Ix, _Id, T) ->
    T;
update_existing({ok, B}, Ix, Id, T) ->
    put_bucket(Ix, macula_dht_bucket:remove(Id, B), T).

-spec touch(macula_dht_xor:id(), table(), integer()) -> table().
touch(<<_:256>> = Id, #{self_id := Self} = T, Now) ->
    touch_at(macula_dht_xor:bucket_index(Self, Id), Id, T, Now).

-spec touch_at(macula_dht_xor:bucket_ix(), macula_dht_xor:id(),
               table(), integer()) -> table().
touch_at(-1, _Id, T, _Now) -> T;
touch_at(Ix, Id, #{buckets := Bs} = T, Now) ->
    touch_existing(maps:find(Ix, Bs), Ix, Id, T, Now).

-spec touch_existing({ok, macula_dht_bucket:bucket()} | error,
                     macula_dht_xor:bucket_ix(), macula_dht_xor:id(),
                     table(), integer()) -> table().
touch_existing(error, _Ix, _Id, T, _Now) ->
    T;
touch_existing({ok, B}, Ix, Id, T, Now) ->
    put_bucket(Ix, macula_dht_bucket:touch(Id, B, Now), T).

%%=====================================================================
%% Internal
%%=====================================================================

-spec find_at(macula_dht_xor:bucket_ix(), macula_dht_xor:id(), table()) ->
          {ok, macula_dht_entry:entry()} | error.
find_at(-1, _Id, _T) ->
    error;
...
-spec find_in({ok, macula_dht_bucket:bucket()} | error,
              macula_dht_xor:id()) ->
          {ok, macula_dht_entry:entry()} | error.
find_in(error, _Id)   -> error;
find_in({ok, B}, Id)  -> macula_dht_bucket:find(Id, B).

-spec put_bucket(macula_dht_xor:bucket_ix(), macula_dht_bucket:bucket(),
                 table()) -> table().
put_bucket(Ix, B, #{buckets := Bs} = T) ->
    T#{buckets := store_or_drop(macula_dht_bucket:size(B), Ix, B, Bs)}.

-spec store_or_drop(non_neg_integer(), macula_dht_xor:bucket_ix(),
                    macula_dht_bucket:bucket(),
                    #{macula_dht_xor:bucket_ix() => macula_dht_bucket:bucket()}) ->
          #{macula_dht_xor:bucket_ix() => macula_dht_bucket:bucket()}.
