...
%% caller (typically `macula_dht_store') publishes the record with
...
-spec place(macula_dht_xor:id(), [macula_frame:station_ref()]) ->
        placement_result().
place(Key, Candidates) ->
    place(Key, Candidates, default_constraints()).

-spec place(macula_dht_xor:id(),
