...
%% full `macula_dht_store:store/3' placement + quorum orchestrator.
%%
...
-type opts() :: #{
    ...
    store_opts  => macula_dht_store:opts()
}.
...
-spec default_store_opts() -> macula_dht_store:opts().
default_store_opts() ->
...
maybe_republish(true, Record,
    ...
    ok    = macula_dht:put_record(Dht, Fresh),
    account(macula_dht:store(Dht, Fresh, StoreOpts), bump(Acc, owned)).

-spec account(macula_dht_store:result(), outcome()) -> outcome().
