...
%%       predicate handed to `macula_relay').</li>
%%   <li>On receiving a CALL frame, runs `macula_relay:process_call/2'
...
%% end to end (`macula_routing` for paths, `macula_source_route`
%% for the wire header, `macula_relay` for per-hop processing,
%% `macula_call_retry` for budget + path rotation,
...
%% the timeout fires. Returns a `macula_call:outcome()'.
...
%% @doc Wrap `call_via_path' in `macula_call_retry:execute/1' for
...
call_via_paths(Net, FromName, PathLists, Spec, RetryOpts) ->
    ...
    macula_call_retry:execute(maps:merge(#{
        paths  => PathLists,
        try_fn => TryFn
    }, RetryOpts)).
...
handle_frame(#{frame_type := call} = Frame, State) ->
    ...
    Action = macula_relay:process_call(Frame, Ctx),
