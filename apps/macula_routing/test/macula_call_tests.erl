%% EUnit tests for macula_call — the CALL state machine.
-module(macula_call_tests).
...
init_starts_in_resolving_after_internal_start_test() ->
    ...
    timer:sleep(5),
    ?assertEqual(resolving, macula_call:state(P)),
    macula_call:stop(P).

info_reports_call_id_and_state_test() ->
    {ok, P} = start_call(),
    timer:sleep(5),
    Info = macula_call:info(P),
...
happy_path_to_succeeded_test() ->
    ...
    Tgt = node_id(),
    Path = [node_id(), Tgt],
    ok = macula_call:resolved(P, Tgt, Path),
    ok = macula_call:selected(P, Tgt),
    ok = macula_call:connected(P),
    ok = macula_call:ack(P, <<"reply payload">>),
    {ok, <<"reply payload">>} = macula_call:await(P, 1_000),
    ?assertEqual(succeeded, macula_call:state(P)),
    Info = macula_call:info(P),
...
resolving_timeout_yields_temporary_relay_failure_test() ->
    ...
    {error, temporary_relay_failure, Detail} = macula_call:await(P, 500),
    ?assertEqual(resolving, maps:get(phase, Detail)),
    ?assertEqual(failed, macula_call:state(P)),
    macula_call:stop(P).
...
selected_target_timeout_yields_unknown_next_peer_test() ->
    ...
    ok = macula_call:resolved(P, node_id(), [node_id()]),
    {error, unknown_next_peer, _} = macula_call:await(P, 500),
    ?assertEqual(failed, macula_call:state(P)),
    macula_call:stop(P).
...
connecting_timeout_yields_temporary_relay_failure_test() ->
    ...
    Tgt = node_id(),
    ok = macula_call:resolved(P, Tgt, [Tgt]),
    ok = macula_call:selected(P, Tgt),
    {error, temporary_relay_failure, Detail} = macula_call:await(P, 500),
...
awaiting_ack_timeout_yields_expiry_too_soon_test() ->
    ...
    ok = macula_call:selected(P, Tgt),
    ok = macula_call:connected(P),
    {error, expiry_too_soon, _} = macula_call:await(P, 500),
    macula_call:stop(P).
...
ack_error_propagates_bolt4_atom_test() ->
    ...
    Hop = node_id(),
    ok = macula_call:ack_error(P, target_realm_refused, Hop),
    {error, target_realm_refused, Detail} = macula_call:await(P, 500),
    ?assertEqual(Hop, maps:get(offending_hop, Detail)),
    macula_call:stop(P).

ack_error_accepts_integer_code_and_resolves_to_atom_test() ->
    {ok, P} = run_to_awaiting_ack(),
    ok = macula_call:ack_error(P, 16#03, undefined),
    {error, relay_disabled, Detail} = macula_call:await(P, 500),
...
notify_pid_receives_success_message_test() ->
    ...
    ok = macula_call:selected(P, Tgt),
    ok = macula_call:connected(P),
    ok = macula_call:ack(P, ok),
    receive
        {macula_call, P, {ok, ok}} -> ok
    after 500 ->
        ?assert(false)
    end,
    macula_call:stop(P).
...
notify_pid_receives_failure_message_test() ->
    ...
    {ok, P} = start_call(#{resolving_timeout_ms => 30},
                        #{notify_pid => Self}),
    receive
        {macula_call, P, {error, temporary_relay_failure, _}} -> ok
    after 500 ->
        ?assert(false)
    end,
    macula_call:stop(P).
...
await_after_terminal_returns_outcome_test() ->
    {ok, P} = start_call(#{resolving_timeout_ms => 30}),
    {error, temporary_relay_failure, _} = macula_call:await(P, 500),
    %% Second await against an already-terminal FSM should still
    %% return the same outcome.
    {error, temporary_relay_failure, _} = macula_call:await(P, 100),
    macula_call:stop(P).
...
multiple_awaiters_all_receive_outcome_test() ->
    ...
    Self = self(),
    Spawn = fun() ->
        Outcome = macula_call:await(P, 1_000),
...
resolved_in_connecting_is_silently_ignored_test() ->
    ...
    ok = macula_call:resolved(P, Tgt, [Tgt]),
    timer:sleep(10),
    ?assertEqual(connecting, macula_call:state(P)),
    macula_call:stop(P).
...
run_to_awaiting_ack() ->
    ...
    ok = macula_call:connected(P),
    timer:sleep(10),
    ?assertEqual(awaiting_ack, macula_call:state(P)),
    {ok, P}.
