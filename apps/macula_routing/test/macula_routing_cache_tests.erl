%% EUnit tests for macula_routing_cache.
-module(macula_routing_cache_tests).

-include_lib("eunit/include/eunit.hrl").

start() ->
    {ok, P} = macula_routing_cache:start_link(#{ttl_ms => 60_000,
                                                sweep_interval_ms => 60_000}),
    P.

start(Opts) ->
    {ok, P} = macula_routing_cache:start_link(Opts),
    P.

stop(P) -> macula_routing_cache:stop(P).
...
lookup_on_empty_returns_miss_test() ->
    P = start(),
    ?assertEqual(miss, macula_routing_cache:lookup(P, dest1)),
    ?assertEqual(0, macula_routing_cache:size(P)),
    stop(P).
...
store_then_lookup_returns_paths_test() ->
    P = start(),
    Paths = [[a, b, c], [a, d, c]],
    ok = macula_routing_cache:store(P, c, Paths),
    ?assertEqual({ok, Paths}, macula_routing_cache:lookup(P, c)),
    ?assertEqual(1, macula_routing_cache:size(P)),
    stop(P).

store_overwrites_existing_test() ->
    P = start(),
    ok = macula_routing_cache:store(P, dest, [[a, b, dest]]),
    ok = macula_routing_cache:store(P, dest, [[a, c, dest]]),
    ?assertEqual({ok, [[a, c, dest]]},
                 macula_routing_cache:lookup(P, dest)),
    stop(P).

all_destinations_lists_keys_test() ->
    P = start(),
    ok = macula_routing_cache:store(P, x, [[a, x]]),
    ok = macula_routing_cache:store(P, y, [[a, y]]),
    ?assertEqual([x, y], lists:sort(macula_routing_cache:all_destinations(P))),
    stop(P).
...
lookup_after_ttl_returns_miss_and_drops_entry_test() ->
    ...
    ok = macula_routing_cache:store(P, dest, [[a, dest]]),
    timer:sleep(60),
    ?assertEqual(miss, macula_routing_cache:lookup(P, dest)),
...
evict_expired_returns_evicted_destinations_test() ->
    ...
    ok = macula_routing_cache:store(P, y, [[a, y]]),
    timer:sleep(60),
    Removed = macula_routing_cache:evict_expired(P),
...
invalidate_drops_specific_destination_test() ->
    ...
    ok = macula_routing_cache:store(P, x, [[a, x]]),
    ok = macula_routing_cache:store(P, y, [[a, y]]),
    ok = macula_routing_cache:invalidate(P, x),
    ?assertEqual(miss, macula_routing_cache:lookup(P, x)),
    ?assertEqual({ok, [[a, y]]}, macula_routing_cache:lookup(P, y)),
    stop(P).

invalidate_unknown_destination_is_noop_test() ->
    P = start(),
    ok = macula_routing_cache:invalidate(P, never_stored),
    ?assertEqual(0, macula_routing_cache:size(P)),
    stop(P).
...
invalidate_via_hop_drops_paths_through_failed_hop_test() ->
    ...
    %% only evict dest1.
    ok = macula_routing_cache:store(P, dest1, [[a, hop_b, dest1]]),
    ok = macula_routing_cache:store(P, dest2, [[a, hop_c, dest2]]),
    Removed = macula_routing_cache:invalidate_via_hop(P, hop_b),
    ?assertEqual([dest1], Removed),
    ?assertEqual(miss, macula_routing_cache:lookup(P, dest1)),
    ?assertMatch({ok, _}, macula_routing_cache:lookup(P, dest2)),
    stop(P).
...
invalidate_via_hop_evicts_when_any_disjoint_path_uses_hop_test() ->
    ...
    %% trustworthy.
    P = start(),
    ok = macula_routing_cache:store(P, dest,
                                    [[a, hop_x, dest], [a, hop_y, dest]]),
    Removed = macula_routing_cache:invalidate_via_hop(P, hop_x),
...
invalidate_via_hop_unknown_hop_is_noop_test() ->
    P = start(),
    ok = macula_routing_cache:store(P, dest, [[a, b, dest]]),
    ?assertEqual([], macula_routing_cache:invalidate_via_hop(P, unrelated)),
    ?assertEqual(1, macula_routing_cache:size(P)),
    stop(P).
...
invalidate_via_dest_node_evicts_test() ->
    ...
    P = start(),
    ok = macula_routing_cache:store(P, dest, [[a, b, dest]]),
    Removed = macula_routing_cache:invalidate_via_hop(P, dest),
    ?assertEqual([dest], Removed),
    stop(P).
...
auto_sweep_evicts_in_background_test() ->
    ...
    ?assertEqual(0, macula_routing_cache:size(P)),
    Stats = macula_routing_cache:stats(P),
