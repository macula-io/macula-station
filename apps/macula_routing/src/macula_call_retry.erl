...
%% `try_fn :: fun((path()) -> macula_call:outcome())' callback that
...
-type try_fn()  :: fun((path()) -> macula_call:outcome()).
...
-spec decide(macula_call:outcome(), [path()], try_fn(),
             non_neg_integer(), map()) -> result().
...
-spec follow_retry(boolean(), macula_call:outcome(), [path()],
