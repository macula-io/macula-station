...
%% `macula_content_store' into signed `content_announcement' records
%% put into the local DHT instance.
%%
%% Subscribes to the pg group `macula_content_store:events_group/0',
%% receives `{manifest_stored, MCID, Manifest}' messages, and
%% publishes via `macula_dht:put_record/2'. Records are signed with
%% the configured station identity so DHT consumers can verify
%% authenticity end-to-end.
%%
%% Configured at start-up with:
%% <ul>
%%   <li>`dht'      — `macula_dht:dht()' reference (the local DHT
...
%% `macula_content_store' fan-outs `{manifest_stored, ...}' messages
...
%% its identity's DHT.
-module(macula_content_announcer).
...
init(Opts) ->
    ...
    ok = pg:join(macula_content_store:events_group(), self()),
    {ok, #state{
...
publish(Dht, Record) ->
    case catch macula_dht:put_record(Dht, Record) of
        ok                  -> ok;
