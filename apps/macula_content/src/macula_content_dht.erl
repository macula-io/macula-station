...
%% glue code) wraps these helpers around `macula_dht:put_record/2'
%% and `macula_dht:find_value/3' calls so this module stays free of
...
%% macula-station's DHT API is record-oriented (signed envelopes via
...
%% `macula_record' envelope and feeding it to `macula_dht:put_record/2'.
