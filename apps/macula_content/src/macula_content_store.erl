...
match_any_hash(Data, Hash) ->
    macula_content_hasher:hash(blake3, Data) =:= Hash
      orelse macula_content_hasher:hash(sha256, Data) =:= Hash.
...
%% messages. Subscribers (e.g. `macula_content_announcer') join this
%% group and react.
-spec events_group() -> atom().
events_group() ->
    macula_content_events.
...
default_base_dir() ->
    case application:get_env(macula_content, store_dir) of
        {ok, Dir} -> Dir;
