...
chunk_info(Chunks, Algorithm) ->
    {Infos, _} = lists:foldl(
        fun(C, {Acc, {Idx, Off}}) ->
            Sz = byte_size(C),
            H  = macula_content_hasher:hash(Algorithm, C),
...
combine([Last], Algorithm, Acc) ->
    ...
    H = macula_content_hasher:hash(Algorithm, <<Last/binary, Last/binary>>),
    lists:reverse([H | Acc]);
combine([L, R | Rest], Algorithm, Acc) ->
    H = macula_content_hasher:hash(Algorithm, <<L/binary, R/binary>>),
