...
mcid_default_algorithm() ->
    <<_:8, Codec:8, _:32/binary>> = macula_content:mcid(<<"x">>),
    ?assertEqual(16#55, Codec).

mcid_explicit_sha256() ->
    M = macula_content:mcid(<<"x">>, sha256),
    ?assertEqual(34, byte_size(M)).

mcid_string_roundtrip() ->
    {ok, Manifest} = macula_content:create_manifest(<<"hi">>),
    Mcid = maps:get(mcid, Manifest),
    Str  = macula_content:mcid_to_string(Mcid),
    ?assertEqual({ok, Mcid}, macula_content:mcid_from_string(Str)).

create_and_verify_manifest() ->
    Data = <<"verify">>,
    {ok, M} = macula_content:create_manifest(Data),
    ?assert(macula_content:verify_manifest(M, Data)),
    ?assertNot(macula_content:verify_manifest(M, <<"changd">>)).

missing_chunks_for_unstored_data() ->
    Data = <<"chunked-data-here">>,
    {ok, M} = macula_content:create_manifest(Data, #{chunk_size => 4}),
    %% Nothing in the store yet, so all chunks are missing.
    ?assertEqual(maps:get(chunks, M), macula_content:missing_chunks(M)).

store_then_fetch_roundtrip() ->
    Data = <<"store-and-fetch-content">>,
    {ok, MCID} = macula_content:store(Data),
    ?assertEqual({ok, Data}, macula_content:fetch(MCID)).

store_with_options() ->
    {ok, MCID} = macula_content:store(<<"x">>,
                                       #{name => <<"named.txt">>,
                                         chunk_size => 1}),
    {ok, M} = macula_content_store:get_manifest(MCID),
...
is_local_for_block_mcid() ->
    ...
    ?assertNot(macula_content:is_local(BlockMcid)),
    ok = macula_content_store:put_block(BlockMcid, Data),
    ?assert(macula_content:is_local(BlockMcid)).

is_local_for_manifest_mcid() ->
    Data = <<"manifested">>,
    {ok, MCID} = macula_content:store(Data),
    ?assert(macula_content:is_local(MCID)).

status_returns_stats_map() ->
    Stats = macula_content:status(),
