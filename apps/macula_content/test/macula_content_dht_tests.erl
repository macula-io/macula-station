...
dht_key_is_32_bytes_test() ->
    ?assertEqual(32, byte_size(macula_content_dht:dht_key(mcid()))).

dht_key_is_deterministic_test() ->
    M = mcid(),
    ?assertEqual(macula_content_dht:dht_key(M),
                 macula_content_dht:dht_key(M)).

dht_key_differs_per_mcid_test() ->
    ?assertNotEqual(macula_content_dht:dht_key(mcid()),
                    macula_content_dht:dht_key(mcid())).

create_provider_info_required_fields_test() ->
    P = macula_content_dht:create_provider_info(
...
format_providers_handles_single_test() ->
    ...
    [Norm] = macula_content_dht:format_providers(P),
...
format_providers_handles_list_test() ->
    ...
    Norms = macula_content_dht:format_providers(Ps),
    ?assertEqual(2, length(Norms)).

format_providers_handles_empty_test() ->
    ?assertEqual([], macula_content_dht:format_providers([])).

create_announcement_returns_key_value_pair_test() ->
    {Key, Value} = macula_content_dht:create_announcement(
                     mcid(), node_id(), <<"e">>, #{name => <<"x">>}),
    ?assertEqual(32, byte_size(Key)),
    ?assertEqual(300, maps:get(ttl, Value)).

create_removal_marks_removed_test() ->
    R = macula_content_dht:create_removal(node_id()),
    ?assertEqual(true, maps:get(removed, R)).

default_ttl_is_300_test() ->
    ?assertEqual(300, macula_content_dht:default_ttl()).

get_ttl_uses_opt_or_default_test() ->
    ?assertEqual(60,  macula_content_dht:get_ttl(#{ttl => 60})),
    ?assertEqual(300, macula_content_dht:get_ttl(#{})).

reannounce_interval_below_floor_test() ->
    ?assertEqual(30, macula_content_dht:reannounce_interval(60)).

reannounce_interval_above_threshold_test() ->
    ?assertEqual(240, macula_content_dht:reannounce_interval(300)).
