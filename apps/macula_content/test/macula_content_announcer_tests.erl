-module(macula_content_announcer_tests).
...
setup() ->
    Dir = filename:join("/tmp",
            "macula-content-announcer-"
            ++ integer_to_list(erlang:unique_integer([positive]))),
    {ok, Store} = macula_content_store:start_link(#{store_path => Dir}),
    Kp = macula_identity:generate(),
    {ok, Announcer} = macula_content_announcer:start_link(#{
        dht        => undefined,
        identity   => Kp,
...
cleanup({Store, Announcer, _Kp, Dir}) ->
    catch_stop_alive(Announcer, fun(P) -> macula_content_announcer:stop(P) end),
    catch_stop_alive(Store,     fun(_P) -> macula_content_store:stop() end),
...
announcer_joins_event_group(Announcer) ->
    Members = pg:get_members(macula_content_store:events_group()),
...
announce_with_undefined_dht_returns_error(Announcer) ->
    ...
    ?assertEqual({error, dht_not_configured},
                 macula_content_announcer:announce(
...
manifest_stored_event_reaches_announcer(Announcer) ->
    ...
    {ok, M} = macula_content_manifest:create(<<"event-test">>),
...
announce_by_mcid_uses_local_manifest(Announcer) ->
    {ok, M} = macula_content_manifest:create(<<"by-mcid">>),
    ok = macula_content_store:put_manifest(M),
    ...
    %% answer.
    Result = macula_content_announcer:announce(Announcer, maps:get(mcid, M)),
...
announce_by_unknown_mcid_returns_not_found(Announcer) ->
    ...
    ?assertEqual({error, not_found},
                 macula_content_announcer:announce(Announcer, Unknown)).
