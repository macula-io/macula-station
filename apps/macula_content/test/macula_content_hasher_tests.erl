...
sha256_hash_matches_crypto_test() ->
    ...
    ?assertEqual(crypto:hash(sha256, Data),
                 macula_content_hasher:hash(sha256, Data)).

blake3_hash_is_32_bytes_test() ->
    Hash = macula_content_hasher:hash(blake3, <<"hello">>),
    ?assertEqual(32, byte_size(Hash)).

blake3_hash_streaming_matches_single_call_test() ->
    Chunks = [<<"hello ">>, <<"world">>],
    Whole  = iolist_to_binary(Chunks),
    ?assertEqual(macula_content_hasher:hash(blake3, Whole),
                 macula_content_hasher:hash_streaming(blake3, Chunks)).

sha256_streaming_matches_single_call_test() ->
    Chunks = [<<"foo ">>, <<"bar ">>, <<"baz">>],
    Whole  = iolist_to_binary(Chunks),
    ?assertEqual(macula_content_hasher:hash(sha256, Whole),
                 macula_content_hasher:hash_streaming(sha256, Chunks)).
...
verify_returns_false_for_wrong_hash_test() ->
    Wrong = <<0:256>>,
    ?assertNot(macula_content_hasher:verify(blake3, <<"abc">>, Wrong)).

supported_algorithms_includes_both_test() ->
    Algos = macula_content_hasher:supported_algorithms(),
...
default_algorithm_is_blake3_test() ->
    ?assertEqual(blake3, macula_content_hasher:default_algorithm()).

hex_encode_known_input_test() ->
    ?assertEqual(<<"deadbeef">>,
                 macula_content_hasher:hex_encode(<<16#de, 16#ad, 16#be, 16#ef>>)).

hex_decode_roundtrip_test() ->
    Bin = <<1, 2, 3, 4, 5>>,
    Hex = macula_content_hasher:hex_encode(Bin),
    ?assertEqual({ok, Bin}, macula_content_hasher:hex_decode(Hex)).

hex_decode_uppercase_works_test() ->
    ?assertEqual({ok, <<16#DE, 16#AD>>},
                 macula_content_hasher:hex_decode(<<"DEAD">>)).

hex_decode_invalid_input_fails_test() ->
    ?assertEqual({error, invalid_hex},
                 macula_content_hasher:hex_decode(<<"xyz!">>)),
    ?assertEqual({error, invalid_hex},
                 macula_content_hasher:hex_decode(<<"abc">>)).
