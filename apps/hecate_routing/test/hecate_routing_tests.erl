%% EUnit tests for hecate_routing.
-module(hecate_routing_tests).
...
new_is_empty_test() ->
    G = hecate_routing:new(),
    ?assertEqual([], hecate_routing:vertices(G)).

add_vertex_idempotent_test() ->
    G  = hecate_routing:add_vertex(a, hecate_routing:new()),
    G2 = hecate_routing:add_vertex(a, G),
    ?assertEqual([a], hecate_routing:vertices(G2)).

add_edge_creates_endpoints_and_weight_test() ->
    G = hecate_routing:add_edge(a, b, 5, hecate_routing:new()),
    ?assertEqual(lists:sort([a, b]),
                 lists:sort(hecate_routing:vertices(G))),
    ?assertEqual({ok, 5}, hecate_routing:weight(a, b, G)).

add_edge_self_loop_rejected_test() ->
    ?assertError(function_clause,
                 hecate_routing:add_edge(a, a, 1, hecate_routing:new())).

add_edge_negative_weight_rejected_test() ->
    ?assertError(function_clause,
                 hecate_routing:add_edge(a, b, -1, hecate_routing:new())).

add_edges_bulk_inserts_test() ->
    G = hecate_routing:add_edges(hecate_routing:new(),
                                 [{a, b, 1}, {b, c, 2}, {c, d, 3}]),
    ?assertEqual(4, length(hecate_routing:vertices(G))).

weight_unknown_returns_error_test() ->
    G = hecate_routing:new(),
    ?assertEqual(error, hecate_routing:weight(a, b, G)).
...
tier_penalty_orders_t0_above_t3_test() ->
    ?assert(hecate_routing:tier_penalty(t0)
              > hecate_routing:tier_penalty(t1)),
    ?assert(hecate_routing:tier_penalty(t1)
              > hecate_routing:tier_penalty(t2)),
    ?assert(hecate_routing:tier_penalty(t2)
              > hecate_routing:tier_penalty(t3)).

edge_weight_with_known_latency_test() ->
    %% 30 + 10 (default bw) + 5 (t1) = 45
    ?assertEqual(45, hecate_routing:edge_weight(t1, 30)).

edge_weight_with_undefined_latency_uses_default_test() ->
    %% 100 + 10 + 5 = 115
    ?assertEqual(115, hecate_routing:edge_weight(t1, undefined)).

edge_weight_custom_bandwidth_term_test() ->
    %% 30 + 25 + 1 (t3) = 56
    ?assertEqual(56, hecate_routing:edge_weight(t3, 30, 25)).
...
shortest_path_in_diamond_picks_lower_cost_branch_test() ->
    ...
    {ok, Path, Cost} = hecate_routing:shortest_path(G, a, d),
...
shortest_path_uses_relaxation_through_indirect_route_test() ->
    ...
    {ok, Path, _} = hecate_routing:shortest_path(G, a, d),
    ?assertEqual([a, b, d], Path).

shortest_path_unreachable_returns_none_test() ->
    G = hecate_routing:add_vertex(c,
                                  hecate_routing:add_edge(a, b, 1,
                                                          hecate_routing:new())),
    ?assertEqual(none, hecate_routing:shortest_path(G, a, c)).

shortest_path_unknown_source_returns_none_test() ->
    G = build([{a, b, 1}]),
    ?assertEqual(none, hecate_routing:shortest_path(G, x, b)).

shortest_path_src_equals_tgt_test() ->
    G = hecate_routing:add_vertex(a, hecate_routing:new()),
    {ok, [a], 0} = hecate_routing:shortest_path(G, a, a).
...
disjoint_paths_yields_two_in_diamond_test() ->
    ...
    Paths = hecate_routing:disjoint_paths(G, a, e, 2),
...
disjoint_paths_yields_three_when_three_routes_exist_test() ->
    G = build([{a, b, 1}, {b, e, 1},
               {a, c, 1}, {c, e, 1},
               {a, d, 1}, {d, e, 1}]),
    Paths = hecate_routing:disjoint_paths(G, a, e, 3),
...
disjoint_paths_first_path_is_shortest_test() ->
    ...
    G = build([{a, b, 10}, {b, e, 10},
               {a, c, 1},  {c, e, 1}]),
    [First, Second] = hecate_routing:disjoint_paths(G, a, e, 2),
...
build(Edges) ->
    hecate_routing:add_edges(hecate_routing:new(), Edges).
