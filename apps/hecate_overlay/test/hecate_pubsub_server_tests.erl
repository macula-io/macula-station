...
%%---------------------------------------------------------------------
%% relay_event — per-hop re-sign + dedup (mesh-realm relay path)
%%---------------------------------------------------------------------
...
relay_event_dedups_on_publisher_seq_test() ->
    ...
    ?assertMatch({_, [_]}, hecate_pubsub_server:relay_event(Pid, Frame2)),
    hecate_pubsub_server:stop(Pid).
...
relay_publish_unchanged_no_dedup_test() ->
    %% relay_publish does NOT use the dedup cache (only relay_event
    %% does, and only for the mesh realm). PUBLISH frames from
    %% daemons don't loop, so dedup is unnecessary here.
    R = realm(),
    {ok, Pid} = hecate_pubsub_server:start_link(
                  #{realm => R, identity => keypair()}),
    ok = hecate_pubsub_server:subscribe(Pid, <<"t">>, id(1)),
    ...
    ?assertMatch({_, [_]}, hecate_pubsub_server:relay_publish(Pid, Frame)),
    %% Second call with same (publisher, seq): still processed
    %% (different from relay_event, which would dedup).
    ?assertMatch({_, [_]}, hecate_pubsub_server:relay_publish(Pid, Frame)),
    hecate_pubsub_server:stop(Pid).
