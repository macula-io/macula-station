-module(hecate_station_bootstrap_runner_tests).
...
runner_cascade_failure_is_wrapped_test_() ->
    {setup, fun start_dht/0, fun stop_dht/1, fun({Dht}) ->
        fun() ->
            ...
            Tiers = [{hecate_station_stub_tier, #{error => boom}}],
            Cfg   = #{tiers => Tiers,
...
runner_reads_application_env_test_() ->
    {setup, fun start_dht/0, fun stop_dht/1, fun({Dht}) ->
        fun() ->
            Peers = hecate_station_stub_tier:stub_peers(2),
            ...
            try
                {ok, #{summary := #{admitted := 2}}} =
                    hecate_station_bootstrap_runner:run(Dht)
            after
                application:unset_env(hecate_bootstrap, tiers),
                application:unset_env(hecate_bootstrap, cascade_opts)
            end
        end
    end}.
