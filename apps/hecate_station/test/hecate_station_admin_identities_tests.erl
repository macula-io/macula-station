...
%% `hecate_station_sup' registered name.
-module(hecate_station_admin_identities_tests).
...
setup_app_with(TokenSpec) ->
    ...
    {ok, AdminBoundPort} = hecate_station:admin_addr(),
...
save_env() ->
    ...
    [{K, application:get_env(hecate_station, K)} || K <- Keys].
