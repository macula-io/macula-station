...
%%
%% Spawns two `hecate_station_identity_sup' instances inside one
...
-module(hecate_station_identity_quic_tests).
...
two_identities_cross_dial(Dir) ->
    ...
    try
        DhtA  = child_pid(SupA, hecate_dht),
        DhtB  = child_pid(SupB, hecate_dht),
        SwimA = child_pid(SupA, hecate_swim),
        SwimB = child_pid(SupB, hecate_swim),

        %% Both per-identity DHTs are alive and disjoint.
        ?assertNotEqual(DhtA, DhtB),
        ?assertEqual(0, hecate_dht:size(DhtA)),
        ?assertEqual(0, hecate_dht:size(DhtB)),
        ...
        ok = wait_until(fun() ->
            hecate_dht:contains(DhtB, IdA) andalso
            lists:any(fun(#{node_id := N}) -> N =:= IdA end,
                      hecate_swim:members(SwimB))
        end, 10_000),

        ?assertEqual(0, hecate_dht:size(DhtA)),
        ?assertEqual(1, hecate_dht:size(DhtB)),
        %% IdB must NOT have leaked into A's DHT.
        ?assertNot(hecate_dht:contains(DhtA, IdB)),
        %% A's SWIM must not have learnt anything either.
        ?assertEqual([], hecate_swim:members(SwimA)),
