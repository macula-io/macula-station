%% EUnit tests for `hecate_station_record_fanout'.
%%
%% Mirrors the classification surface previously covered by
%% `hecate_station_fact_publisher_tests'. The fan-out replaces the
...
%% `hecate_station_identity_registry'. Each test fixture wires:
%%
...
-module(hecate_station_record_fanout_tests).
...
setup_single() ->
    ...
    {ok, Pub} = hecate_station_record_fanout:start_link(#{
...
cleanup_single(#{publisher := Pub, registry := Reg, observer := Obs}) ->
    catch hecate_station_record_fanout:stop(Pub),
...
unknown_record_type_is_ignored(#{publisher := Pub, registry := Reg,
    ...
    hecate_station_record_fanout:on_record(Pub, IdKey, FakeRecord),
    receive
...
record_fans_to_both_identities(#{publisher := Pub,
    ...
    hecate_station_record_fanout:on_record(Pub, <<"id1">>, station_record()),
...
assert_publish_lands(#{publisher := Pub, registry := Reg, observer := Obs,
    ...
    hecate_station_record_fanout:on_record(Pub, IdKey, Record),
