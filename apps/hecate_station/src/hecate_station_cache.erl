...
%%
%% ```
%% {hecate_station_cache, 1, [Entry1, Entry2, ...]}
%% '''
%%
...
%%
%% <ul>
%%   <li>Boot — `hecate_station_app:start/2' calls `load/2'
...
%% </ul>
%%
%% Entries are encoded by their `hecate_dht_entry:spec()' shape so
%% that `load/2' can simply iterate and call `hecate_dht:observe/2'.
%% Age-based fields (`first_seen', `last_seen', uptime) are dropped on
%% write — they will be recomputed when the entry is re-observed.
-module(hecate_station_cache).
...
-define(FORMAT_TAG, hecate_station_cache).
-define(FORMAT_VSN, 1).

-type opts() :: #{
    dht   := pid(),
    cache := hecate_station_config:cache_cfg()
}.
...
-record(state, {
    dht       :: pid(),
    cache_cfg :: hecate_station_config:cache_cfg()
}).
...
collect_entries(Dht) ->
    [E || {_Ix, Bucket} <- hecate_dht:populated_buckets(Dht),
          E <- hecate_dht_bucket:members(Bucket)].
...
step_ingest({ok, Spec}, Dht, #{loaded := L} = Acc) ->
    _ = hecate_dht:observe(Dht, Spec),
    Acc#{loaded := L + 1};
step_ingest(skip, _Dht, #{skipped := S} = Acc) ->
    Acc#{skipped := S + 1}.

%% Turn a live `hecate_dht_entry:entry()' into an `observe' spec,
