...
-module(hecate_station_forwarder_sup).
...
-spec start_forwarder(pid(),
                      hecate_station_peering_forwarder:opts()) ->
    {ok, pid()} | {error, term()}.
...
init([]) ->
    ...
    Child = #{id       => hecate_station_peering_forwarder,
              start    => {hecate_station_peering_forwarder, start_link, []},
              restart  => transient,
              shutdown => 5_000,
              type     => worker,
              modules  => [hecate_station_peering_forwarder]},
    {ok, {SupFlags, [Child]}}.
