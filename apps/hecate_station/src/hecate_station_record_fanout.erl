...
%%
%% Replaces the previous per-identity `hecate_station_fact_publisher'
...
%% record_stored cast it queries `hecate_station_identity_registry'
...
-module(hecate_station_record_fanout).
...
-type identity_key() :: hecate_station_identity_registry:identity_key().
-type phase3_info() :: #{
    pubsub_registry := pid(),
    peer_observer   := pid(),
...
%% `hecate_station_identity_registry' so the next `record_stored'
%% cast finds it.
-spec identity_ready(identity_key(), phase3_info()) -> ok.
identity_ready(Key, Info) when is_map(Info) ->
    hecate_station_identity_registry:set_phase3(Key, Info).
...
identity_gone(Key) ->
    hecate_station_identity_registry:clear_phase3(Key).
...
snapshot_identities(#state{fixed_identities = undefined}) ->
    ...
    try hecate_station_identity_registry:phase3_snapshot()
    catch
        exit:{noproc, _}     -> [];
        exit:{shutdown, _}   -> [];
        exit:{normal, _}     -> []
...
%% `hecate_station_fact_publisher'. The sole change is that the
...
deliver([NodeId | Rest], Frame, Obs) ->
    deliver_to(hecate_station_peer_observer:conn_for(Obs, NodeId), Frame),
