...
%% AFTER both `hecate_handler_registry' and `hecate_dht' are up.
...
-module(hecate_station_dht_handlers).
...
advertise_all(Registry, Dht) ->
    ok = hecate_handler_registry:advertise(
...
on_verify(Dht, _Record, {ok, Verified}) ->
    ok = hecate_dht:put_record(Dht, Verified),
    {ok, ok};
...
handle_find_record(Dht, #{key := Key})
  when is_binary(Key), byte_size(Key) =:= 32 ->
    classify_local_lookup(hecate_dht:find_local_record(Dht, Key));
...
handle_find_records_by_type(Dht, #{type := Type})
  when is_integer(Type), Type >= 0, Type =< 255 ->
    All = hecate_dht:list_records(Dht),
