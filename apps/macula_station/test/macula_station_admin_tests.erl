...
-module(macula_station_admin_tests).
...
setup_app() ->
    ...
    {ok, Port} = macula_station:admin_addr(),
...
%%------------------------------------------------------------------
%% Env wiring — duplicates macula_station_app_tests' helpers so the
...
make_tmpdir() ->
    Base = filename:join(["/tmp", "macula-station-admin-test",
