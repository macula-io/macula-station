...
%%
%% Boots the full station, calls `macula_station:shutdown/0,1',
...
-module(macula_station_shutdown_tests).
...
%% `macula_station:shutdown/0' — minus the sup teardown, which the
...
setup_app() ->
    ...
    _ = kill_if_alive(macula_station_sup),
    ...
    {ok, _Sup} = macula_station_app:start(normal, []),
    {ok, Dht}  = macula_station:dht(),
    {ok, Kp}   = macula_station:current_identity(),
...
tombstone(Kp, Reason) ->
    ...
    Unsigned = macula_record:tombstone(Pub,
                                       macula_station:tombstone_type(),
                                       Reason),
    macula_record:sign(Unsigned, Kp).

%%------------------------------------------------------------------
%% Env + dir helpers — mirror macula_station_admin_tests shapes so
...
make_tmpdir() ->
    Base = filename:join(["/tmp", "macula-station-shutdown-test",
