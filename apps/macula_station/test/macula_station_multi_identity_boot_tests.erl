...
%%
%% Drives `macula_station_app:start/2' through the multi-identity
...
%% `macula_station_identity_registry' and its listener is alive.
-module(macula_station_multi_identity_boot_tests).
...
three_identities_boot() ->
    ...
    try
        ...
        Listed = lists:sort(
            [Hostname || {Hostname, _Pid}
                         <- macula_station_identity_registry:list()]),
    ...
    after
        ...
        application:unset_env(macula_station, port),
        application:unset_env(macula_station, certfile),
        application:unset_env(macula_station, keyfile),
        application:unset_env(macula_station, box_secret_path),
        rm_rf(Dir)
    end.

verify_identity_listener(Hostname) ->
    {ok, IdSup} = macula_station_identity_registry:lookup(Hostname),
...
distinct_keys() ->
    ...
    try
        ...
        {ok, Sup} = macula_station_app:start(normal, []),
        {ok, SupX} = macula_station_identity_registry:lookup(<<"x.macula.io">>),
        {ok, SupY} = macula_station_identity_registry:lookup(<<"y.macula.io">>),

        DhtX  = child_pid(SupX, macula_dht),
        DhtY  = child_pid(SupY, macula_dht),
        SwimX = child_pid(SupX, macula_swim),
        SwimY = child_pid(SupY, macula_swim),
