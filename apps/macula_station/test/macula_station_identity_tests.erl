-module(macula_station_identity_tests).
...
path_for_joins_data_dir_test() ->
    ?assertEqual("/tmp/foo/identity.erl.bin",
                 macula_station_identity:path_for("/tmp/foo")).
...
warm_boot_returns_same_identity_test_() ->
    {setup, fun tmpdir/0, fun rm_rf/1, fun(Dir) ->
        fun() ->
            Path = macula_station_identity:path_for(Dir),
            {ok, Kp1} = macula_station_identity:load_or_generate(Path),
            {ok, Kp2} = macula_station_identity:load_or_generate(Path),
...
missing_dir_recovery_test_() ->
    {setup, fun tmpdir/0, fun rm_rf/1, fun(Dir) ->
        fun() ->
            ...
            Path   = macula_station_identity:path_for(Nested),
            ?assertNot(filelib:is_dir(Nested)),
            {ok, _Kp} = macula_station_identity:load_or_generate(Path),
            ?assert(filelib:is_regular(Path))
        end
    end}.
    ...
    {setup, fun tmpdir/0, fun rm_rf/1, fun(Dir) ->
        fun() ->
            ...
            ?assertEqual({error, enoent},
                         macula_station_identity:load(Path))
        end
    end}.
...
generate_overwrites_even_when_file_exists_test_() ->
    ...
    {setup, fun tmpdir/0, fun rm_rf/1, fun(Dir) ->
        fun() ->
            Path       = macula_station_identity:path_for(Dir),
            {ok, Kp1}  = macula_station_identity:generate(Path),
            {ok, Kp2}  = macula_station_identity:generate(Path),
            ?assertNotEqual(macula_identity:public(Kp1),
                            macula_identity:public(Kp2)),
            {ok, KpR}  = macula_station_identity:load(Path),
...
tmpdir() ->
    Base = filename:join(["/tmp", "macula-station-test",
