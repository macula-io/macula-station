-module(macula_station_rebootstrap_tests).
...
healthy_dht_no_trigger_test() ->
    ...
    try
        Status = macula_station_rebootstrap:force_tick(Rb),
...
brief_dip_does_not_trigger_test() ->
    ...
    try
        ...
        %% partition window.
        macula_station_rebootstrap:force_tick(Rb),
        macula_station_rebootstrap:force_tick(Rb),
        Status = macula_station_rebootstrap:force_tick(Rb),
        ?assertEqual(0, maps:get(triggers, Status)),
...
sustained_low_triggers_once_test() ->
    ...
    try
        ...
        _ = macula_station_rebootstrap:force_tick(Rb),
        S2 = macula_station_rebootstrap:state(Rb),
...
recovery_clears_low_since_test() ->
    ...
    try
        ...
        ?assertNotEqual(undefined,
            maps:get(low_since_ms, macula_station_rebootstrap:state(Rb))),
        %% Fill the DHT above threshold.
        macula_dht:observe(Dht, spec(rand_id())),
        macula_dht:observe(Dht, spec(rand_id())),
        _ = macula_station_rebootstrap:force_tick(Rb),
        Status = macula_station_rebootstrap:state(Rb),
        ?assertEqual(undefined, maps:get(low_since_ms, Status)),
        ?assertEqual(0, maps:get(triggers, Status))
    after
        teardown(Rb, Dht)
...
backoff_grows_the_window_under_sustained_partition_test() ->
    ...
    try
        ...
        S0 = macula_station_rebootstrap:state(Rb),
        ...
        _  = macula_station_rebootstrap:force_tick(Rb),
        ok = receive_triggered(500),
        S3 = macula_station_rebootstrap:state(Rb),
...
recovery_resets_consecutive_counter_test() ->
    ...
    try
        ...
        ?assertEqual(1, maps:get(consecutive,
                                  macula_station_rebootstrap:state(Rb))),
        %% Recovery: DHT fills above threshold.
        macula_dht:observe(Dht, spec(rand_id())),
        macula_dht:observe(Dht, spec(rand_id())),
        _ = macula_station_rebootstrap:force_tick(Rb),
        ok = receive_recovered(500),
        SR = macula_station_rebootstrap:state(Rb),
...
drive_triggers(Rb, N, Base) ->
    _ = macula_station_rebootstrap:force_tick(Rb),
    S = macula_station_rebootstrap:state(Rb),
...
receive_recovered(Ms) ->
    receive
        {macula_station_rebootstrap, recovered, _N} -> ok
    after Ms ->
...
teardown(Rb, Dht) ->
    _ = catch macula_station_rebootstrap:stop(Rb),
    _ = catch macula_dht:stop(Dht),
    ok.

receive_triggered(Ms) ->
    receive
        {macula_station_rebootstrap, rebootstrapped, _} -> ok
    after Ms ->
