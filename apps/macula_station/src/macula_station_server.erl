...
-record(state, {
    config       :: macula_station_config:station_opts(),
...
init(Spec) ->
    on_config_loaded(macula_station_config:load(Spec)).
...
start_swim(#{identity := Kp} = Cfg) ->
    ...
    macula_swim:start_link(SwimOpts).
...
handle_call(swim_members, _From, #state{swim = Swim} = S) ->
    {reply, macula_swim:members(Swim), S};
...
handle_info({macula_swim, member_state, _NodeId, _State}, S) ->
...
on_peer_connected(ConnPid, PeerNodeId, #state{swim = Swim} = S) ->
    macula_swim:add_peer(Swim, PeerNodeId, ConnPid),
...
maybe_remove_swim_peer(ConnPid, Peers, Swim) ->
    case maps:get(ConnPid, Peers, undefined) of
        #{peer_node_id := NodeId} -> macula_swim:remove_peer(Swim, NodeId);
        _ -> ok
    end.
...
dispatch_swim(From, Frame, Swim) ->
    case macula_frame:verify(Frame, From) of
        {ok, _}          -> macula_swim:handle_frame(Swim, From, Frame);
        {error, _Reason} -> ok
    end.
