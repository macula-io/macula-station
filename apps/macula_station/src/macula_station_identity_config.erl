...
%% `macula_station_identity_registry:register/2'.
%%
%% == Format ==
%%
...
%%  #{...}]
%% '''
-module(macula_station_identity_config).
...
%% `macula_station_identity_keys:derive/2'; constructs the
...
bind_for(#{bind := undefined}, Shared) ->
    ...
    application:get_env(macula_station, bind,
...
%% Both the boot pipeline (`macula_station_app') and the admin API
...
-spec load_box_secret() ->
    {ok, macula_station_identity_keys:box_secret()} | {error, term()}.
load_box_secret() ->
    macula_station_identity_keys:load_or_generate_box_secret(box_secret_path()).

%% @doc Resolved path for the box-secret. Resolution order, first
%% hit wins:
%% <ol>
%%   <li>`application:get_env(macula_station, box_secret_path)'</li>
...
box_secret_path() ->
    classify_box_secret_path(
        application:get_env(macula_station, box_secret_path),
...
%% @doc Read `port', `certfile', `keyfile' from the `macula_station'
%% app env, with V1-compatible OS env-var fallback so macula-station
%% drops into the existing macula-relay V1 docker-compose without
%% editing `sys.config'.
%%
%% Resolution order, first hit wins:
%% <ol>
%%   <li>`application:get_env(macula_station, Key)'</li>
...
resolve_one({Key, EnvVar, Type}) ->
    classify_resolution(application:get_env(macula_station, Key),
