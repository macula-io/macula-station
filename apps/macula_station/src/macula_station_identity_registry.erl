...
%% A single macula-station BEAM can host many identities; this
%% gen_server is the entry point that
%%
%% <ol>
%%   <li>spawns a fresh `macula_station_identity_sup' linked to
...
-module(macula_station_identity_registry).
...
-record(state, {
    ...
    %% `macula_station_record_fanout' queries this to broadcast
    %% record-stored events across all box-local identities.
    by_phase3 = #{} :: #{identity_key()
                         := macula_station_record_fanout:phase3_info()}
}).
...
-spec lookup_opts(identity_key()) ->
    {ok, macula_station_identity_sup:identity_opts()} | {error, not_found}.
lookup_opts(Key) ->
...
-spec set_phase3(identity_key(),
                 macula_station_record_fanout:phase3_info()) -> ok.
...
%% `[{Key, Info}]'. Used by `macula_station_record_fanout' on each
...
do_register(Key, Opts, false, S) ->
    on_sup_started(Key, Opts,
                   macula_station_identity_sup:start_link(Opts),
                   S).
