...
%%
%% A single macula-station BEAM process hosts N identities
...
%%         <li>`macula_content_announcer' (opt-in, when announcer
...
%%       <ul>
%%         <li>`macula_dht' — per-identity Kademlia routing table.</li>
%%         <li>`macula_swim' — per-identity SWIM failure detector,
%%             signing membership frames with the identity's
%%             keypair.</li>
%%         <li>`macula_station_peer_observer' — glue between peering,
%%             DHT, and SWIM. Fed the per-identity DHT + SWIM pids
%%             as opts.</li>
%%         <li>`macula_station_listener' — bound to the identity's
...
%% pipeline `macula_station_app' uses today, but executes
%% per-identity.
%%
...
-type identity_opts() :: #{
    identity_key       := macula_station_identity_registry:identity_key(),
    ...
    %% `macula_bootstrap' app env is preloaded by another test
    ...
    %% by `macula_station_identity_config:spec_to_opts/3' from the
...
content_announcer_child(#{identity     := Kp,
    ...
    [
        #{id       => macula_content_announcer,
          start    => {macula_content_announcer, start_link, [Opts]},
          restart  => permanent,
          shutdown => 5_000,
          type     => worker,
...
%% Lives alongside `macula_handler_registry'; the peer_observer wires
...
remote_advertise_registry_spec(IdentityOpts) ->
    ...
    #{id       => macula_remote_advertise_registry,
      start    => {macula_remote_advertise_registry, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
bloom_exchange_spec(IdentityOpts) ->
    ...
    #{id       => macula_station_bloom_exchange,
      start    => {macula_station_bloom_exchange, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
forwarder_sup_spec() ->
    #{id       => macula_station_forwarder_sup,
      start    => {macula_station_forwarder_sup, start_link, []},
      restart  => permanent,
      shutdown => 5_000,
      type     => supervisor,
...
relay_ping_spec(IdentityOpts) ->
    ...
    #{id       => macula_station_relay_ping,
      start    => {macula_station_relay_ping, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
peering_router_spec(IdentityOpts) ->
    ...
    #{id       => macula_station_peering_router,
      start    => {macula_station_peering_router, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
pg_scope_for(Kp) ->
    ...
    binary_to_atom(<<"macula_station_relay_", (binary:encode_hex(Pub))/binary>>).
...
overlay_seeder_spec(IdentityOpts, ObsPid) ->
    ...
    #{id       => macula_station_overlay_seeder,
      start    => {macula_station_overlay_seeder, start_link, [Opts]},
      restart  => transient,
      shutdown => 5_000,
      type     => worker,
      modules  => [macula_station_overlay_seeder]}.
...
%% cascade reads `macula_bootstrap' app env; when no tiers are
...
%% `macula_station_identity_registry' so the node-singleton
%% `macula_station_record_fanout' picks it up on the next
...
seed_fanout_registration(Sup, Opts, DhtPid, ObsPid) ->
    ...
    ok = macula_station_record_fanout:identity_ready(IdentityKey, Phase3),
...
install_dht_callback(DhtPid, IdentityKey) ->
    ok = macula_dht:set_on_record_stored(
           DhtPid,
           fun(Record) ->
               macula_station_record_fanout:on_record(IdentityKey, Record)
           end).
...
dht_spec(#{identity := Kp}) ->
    ...
    #{id       => macula_dht,
      start    => {macula_dht, start_link, [#{self_id => Self}]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
      modules  => [macula_dht]}.
...
swim_spec(#{identity := Kp}, Sup) ->
    ...
    #{id       => macula_swim,
      start    => {macula_swim, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
      modules  => [macula_swim]}.
...
handler_registry_spec(IdentityOpts) ->
    ...
    #{id       => macula_handler_registry,
      start    => {macula_handler_registry, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
dht_handlers_spec(HrPid, DhtPid, IdentityOpts) ->
    ...
    #{id       => macula_station_dht_handlers,
      start    => {macula_station_dht_handlers, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
announcer_spec(DhtPid, IdentityOpts) ->
    ...
    #{id       => macula_station_announcer,
      start    => {macula_station_announcer, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
replicate_spec(DhtPid) ->
    #{id       => macula_dht_replicate,
      start    => {macula_dht_replicate, start_link, [#{dht => DhtPid}]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
republish_spec(DhtPid, #{identity := Kp}) ->
    ...
    #{id       => macula_dht_republish,
      start    => {macula_dht_republish, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
expire_spec(DhtPid) ->
    #{id       => macula_dht_expire,
      start    => {macula_dht_expire, start_link, [#{dht => DhtPid}]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
...
observer_spec(DhtPid, SwimPid, IdentityOpts) ->
    ...
    #{id       => macula_station_peer_observer,
      start    => {macula_station_peer_observer, start_link, [Opts]},
      restart  => permanent,
      shutdown => 5_000,
      type     => worker,
