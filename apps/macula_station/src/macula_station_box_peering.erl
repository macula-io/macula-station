...
%%%     it into the local `macula_station_facts' pg gossip group via
...
-module(macula_station_box_peering).
...
-define(PG_SCOPE,              macula_station_facts).
...
seed_local_dhts_if_node_record(16#01, Record, DhtPids) ->
    %% Fan out seeds in parallel — `macula_dht:put_record/2' is a
...
box_identity() ->
    case macula_station_identity_config:load_box_secret() of
        {ok, BoxSecret} ->
            macula_station_identity_keys:derive(BoxSecret,
                                                <<"__box_peering__">>);
        _ ->
