...
| 2026-05-05 | **Phase 1 (multi-process CT harness) closed.** `apps/macula_station/test/macula_station_test_cluster.erl` + `_SUITE.erl` now exist and are stable (15+ consecutive green runs after the guardian-process fix landed in `b99498a`). The C1 reproducer at `macula_station_dht_transport_SUITE.erl` documents the no_transport bug as a self-explaining failing test (will pass once Phase 4 closes C1). The harness IS the gating mechanism for converting 🟡 wired-but-untested rows to 🟢 wired-and-tested — every PR that closes a 🟡 row from now on lands with a CT integration test that uses this harness, and updates the row + summary counts in this document. SDK additions: `macula_station:wait_until_ready/1`, `readiness/0`, `diagnostics/0` (pushed `cd2a116`). See `PLAN_PHASE_1_MULTI_PROCESS_CT_HARNESS.md` and `apps/macula_station/test/README.md`. | Claude |
