...
1. ~~Why `cross_station_pubsub` doesn't deliver even with publisher-signed EVENTs~~ — **resolved (mostly) by the cutover.** Root cause: kessel-lo (the subscriber's station) does *not* dial centrum (the publisher's station) — kessel-lo's `outbound_peers` are {haasrode, gasthuisberg, vaartkom}; centrum dials kessel-lo, not the reverse. `macula_station_peer_links:connections/0` returns only *outbound* links, so a station's `peering_router` only subscribes-on-peer over its own outbound links → kessel-lo→centrum interest must propagate ~2 hops, and the EVENT is relayed back over a ≥2-hop path. Without `publisher_sig` that path is structurally dead (verify-fail accident kills hop 2). **With `publisher_sig` (the cutover) it works** — `cross_station_pubsub` passed 4/5 single-case runs (the 1 miss = a residual SUBSCRIBE-interest-propagation timing race vs the probe's `?SUBSCRIBE_SETTLE_MS=1500`). Residual follow-up: tighten cross-station SUBSCRIBE convergence (the on-SUBSCRIBE immediate-propagation via `notify_router_change/0` should make it fast — investigate why it occasionally isn't), and/or bump the probe settle time for the cross-station cases.
2. ~~The `unknown_next_peer` cross-station RPC routing gap~~ — **primary cause fixed.** `macula_station_peer_observer:deliver_advertise` registered the `remote_advertise` entry but didn't kick the `peering_router`, so ADVERTISE re-propagation to peers waited up to one router tick (~2s) per hop — racing the e2e probes' 1.5s `?ADVERTISE_SETTLE_MS` and the daemons' catch-up retries. Now `on_advertise_match` (new registration) and `on_unadvertise_match` call `notify_router_change/0`, mirroring the SUBSCRIBE path; ADVERTISE converges in ~RTTs. (Possible residual: `peering_router`'s ADVERTISE re-propagation may be effectively single-hop — the moduledoc says so, the code's `direct_remote_advertise_set/1` filter (`Adv =/= SelfId`) reads as multi-hop; if a station is >2 hops from an advertiser it would still miss the route. Worth a careful read if `unknown_next_peer` recurs after the redeploy.) Verify with `cross_station_unary_rpc` / `cross_station_streaming_rpc` once the macula-station `:main` rebuild rolls.
